...
TEST(DebugCodec, LeafRoundTripExact) {
  ...
  ASSERT_TRUE(b.try_append_sorted(Slice(std::string("a\0b", 3)), Slice(Cell(1, "A"))));
  ASSERT_TRUE(b.try_append_sorted(Slice("bb"), Slice(Cell(2, "value"))));
  ASSERT_TRUE(b.try_append_sorted(Slice("cc"), Slice(Cell(3, "", true))));
  b.finish(7, 42);
