...
leaf_entry E(const std::string& k, uint64_t slot, const std::string& v) {
  return leaf_entry{k, encode_cell(slot, OpKind::kPut, Slice(v))};
}
leaf_entry Tomb(const std::string& k, uint64_t slot) {
  return leaf_entry{k, encode_cell(slot, OpKind::kDelete)};
}
...
TEST(Delta, BuildAndFindKey) {
  auto* d = BatchDelta::build(5, {E("a", 5, "x"), E("c", 5, "y"), E("e", 5, "z")}, nullptr);
