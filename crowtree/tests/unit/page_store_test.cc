...
TEST(PageStore, MemRoundTrip) {
  ...
  ASSERT_TRUE(s.WriteAt(100, in.data(), in.size()).ok());
  std::vector<uint8_t> out(in.size(), 0);
  ASSERT_TRUE(s.ReadAt(100, out.data(), out.size()).ok());
  EXPECT_EQ(in, out);
...
TEST(PageStore, MemReadPastEndFails) {
  MemPageStore s(1);
  uint8_t b[4];
  EXPECT_FALSE(s.ReadAt(0, b, sizeof(b)).ok());
}

TEST(PageStore, MemOverwrite) {
  MemPageStore s(1);
  std::vector<uint8_t> a{9, 9, 9};
  ASSERT_TRUE(s.WriteAt(0, a.data(), a.size()).ok());
  std::vector<uint8_t> b{1, 2};
  ASSERT_TRUE(s.WriteAt(0, b.data(), b.size()).ok());
  std::vector<uint8_t> out(3, 0);
  ASSERT_TRUE(s.ReadAt(0, out.data(), out.size()).ok());
  EXPECT_EQ(out[0], 1);
  EXPECT_EQ(out[1], 2);
  EXPECT_EQ(out[2], 9);
}
...
TEST(PageStore, FileRoundTripAcrossReopen) {
  ...
  std::vector<uint8_t> in{10, 20, 30, 40};
  {
    ...
    ASSERT_TRUE(s->WriteAt(8, in.data(), in.size()).ok());
  ...
  }
  {
    ...
    ASSERT_TRUE(FilePageStore::Open(path, 4096, &s).ok());
    std::vector<uint8_t> out(in.size(), 0);
    ASSERT_TRUE(s->ReadAt(8, out.data(), out.size()).ok());
    EXPECT_EQ(in, out);
  }
