...
TEST(FramePage, LeafFindAndLowerBound) {
  ...
  LeafFrameBuilder b(frame.data(), pb);
  for (int i = 0; i < 50; ++i) {
    ASSERT_TRUE(b.try_append_sorted(Slice(Key(i * 2)), Slice(Cell(i, "v"))));
  }
  ...
  EXPECT_EQ(v.lower_bound(Slice(Key(21))), 11u);
  EXPECT_EQ(v.lower_bound(Slice(Key(0))), 0u);
  CellView c;
...
TEST(FramePage, LeafCapacityRejectsWhenFull) {
  ...
  for (int i = 0; i < 1000; ++i) {
    if (!b.try_append_sorted(Slice(Key(i)), Slice(Cell(i, "value-bytes")))) {
      break;
    }
    ++appended;
  }
...
TEST(FramePage, BinaryKeysWithNuls) {
  ...
  LeafFrameBuilder b(frame.data(), pb);
  ASSERT_TRUE(b.try_append_sorted(Slice(k1), Slice(Cell(1, std::string("x\0y", 3)))));
  ASSERT_TRUE(b.try_append_sorted(Slice(k2), Slice(Cell(2, "z"))));
  b.finish(1, kInvalidPageId);
  LeafFrameView v(frame.data(), pb);
...
TEST(FramePage, InFrameDeltaOverlay) {
  ...
  ASSERT_TRUE(b.try_append_sorted(Slice("a"), Slice(Cell(1, "A1"))));
  ASSERT_TRUE(b.try_append_sorted(Slice("b"), Slice(Cell(1, "B1"))));
  b.finish(1, kInvalidPageId);
...
TEST(FramePage, OverflowPartialChunkAndCrc) {
  ...
  overflow_frame_build(frame.data(), pb, 1, kInvalidPageId,
                       reinterpret_cast<const uint8_t*>(payload.data()),
                       static_cast<uint32_t>(payload.size()));
  ASSERT_TRUE(frame_validate(frame.data(), pb));
  OverflowFrameView v(frame.data(), pb);
