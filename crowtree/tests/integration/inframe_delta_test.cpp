...
TEST(InFrameDelta, ParityWithDefaultMode) {
  ...
  auto run = [](bool inframe) {
    ...
    for (int r = 0; r < 600; ++r) {
      ...
      ++slot;
      if (rng() % 8 == 0) {
        EXPECT_TRUE(t.apply(slot, Del1(Key(k))).ok());
      } else {
        EXPECT_TRUE(t.apply(slot, Put1(Key(k), "val" + std::to_string(slot))).ok());
      }
      if (r % 3 == 0) {
...
TEST(InFrameDelta, MicrobenchVsCowRebuild) {
  ...
  auto bench = [](bool inframe) {
    ...
    for (int i = 0; i < 300; ++i) {
      ++slot;
      EXPECT_TRUE(t.apply(slot, Put1(Key(i), "seed")).ok());
    }
    EXPECT_TRUE(t.flush().ok());
    auto t0 = std::chrono::steady_clock::now();
    for (int r = 0; r < 2000; ++r) {
      ++slot;
      EXPECT_TRUE(t.apply(slot, Put1(Key(r % 300), "v" + std::to_string(slot))).ok());
