// PT6d: incremental checkpoint writes only dirty pages, retains clean pages'
...
TEST(IncrementalCheckpoint, SecondCheckpointWithoutChangesWritesNothing) {
  ...
  uint64_t first = t.last_checkpoint_pages_written();
  EXPECT_GT(first, 1u);  // a multi-page tree: everything was dirty

  // No mutations between checkpoints -> nothing should be rewritten.
  ASSERT_TRUE(t.Checkpoint(nullptr).ok());
  EXPECT_EQ(t.last_checkpoint_pages_written(), 0u);
}
...
TEST(IncrementalCheckpoint, SingleKeyEditRewritesOnlyItsPath) {
  ...
  ASSERT_TRUE(t.Checkpoint(nullptr).ok());
  uint64_t total = t.last_checkpoint_pages_written();
  ASSERT_GT(total, 4u);  // several leaves + inner level(s)

  // Touch exactly one key, flush, checkpoint again.
  uint64_t slot = 100000;
  ASSERT_TRUE(t.Apply(slot, Put1(Key(7), "updated"), slot).ok());
  ASSERT_TRUE(t.Flush().ok());
  oracle[Key(7)] = "updated";
  ASSERT_TRUE(t.Checkpoint(nullptr).ok());

  uint64_t rewritten = t.last_checkpoint_pages_written();
...
TEST(IncrementalCheckpoint, SpaceIsReusedAcrossManyCheckpoints) {
  ...
  uint64_t early = 0;
  {
    ...
    // Rewrite the SAME small key set every round and checkpoint. With durable-page
    // GC, freed extents from two checkpoints ago are reused, so the file reaches a
    // steady size instead of growing ~linearly in the number of rounds.
    uint64_t slot = 100000;
    for (int round = 0; round < 50; ++round) {
      for (int i : {1, 2, 3, 4, 5}) {
        std::string v = "r" + std::to_string(round);
        ASSERT_TRUE(t.Apply(slot, Put1(Key(i), v), slot).ok());
        ASSERT_TRUE(t.Flush().ok());
        oracle[Key(i)] = v;
        ++slot;
      }
      ASSERT_TRUE(t.Checkpoint(nullptr).ok());
      if (round == 9) early = store.size();
    }
...
TEST(IncrementalCheckpoint, ReopenAfterIncrementalSeesAllValues) {
  ...
  std::map<std::string, std::string> oracle;
  {
    Crowtree t(env, opt);
    Fill(&t, 200, &oracle);
    ASSERT_TRUE(t.Checkpoint(nullptr).ok());  // checkpoint 1: full image

    // Mutate a spread of keys across different leaves, then checkpoint again.
    uint64_t slot = 100000;
    for (int i : {3, 50, 99, 150, 199}) {
      ASSERT_TRUE(t.Apply(slot, Put1(Key(i), "v2_" + std::to_string(i)), slot).ok());
      ASSERT_TRUE(t.Flush().ok());
      oracle[Key(i)] = "v2_" + std::to_string(i);
      ++slot;
    }
    ASSERT_TRUE(t.Checkpoint(nullptr).ok());  // checkpoint 2: incremental
    EXPECT_LT(t.last_checkpoint_pages_written(), 200u);  // not a full rewrite
  }

  // Reopen from the incremental checkpoint: unchanged keys are read from the
  // first checkpoint's region (retained addrs); mutated keys from the second.
  std::unique_ptr<Crowtree> t2;
  ASSERT_TRUE(Crowtree::Open(env, opt, &t2).ok());
  for (const auto& kv : oracle) {
    std::string v;
