...
std::string encode_generic_text(const char *type_name, uint32_t magic, const uint8_t *buf, size_t len)
{
    ...
    oss << "CROW_CT_" << type_name << " magic=0x" << to_hex(reinterpret_cast<const uint8_t *>(&magic), 4);
    oss << " len=" << len << "\n";
...
std::string encode_anchor_text(const uint8_t *buf, size_t len)
{
    ...
    std::ostringstream oss;
    oss << "CROW_CT_ANCHOR magic=0x" << to_hex(reinterpret_cast<const uint8_t *>(&magic), 4);
...
std::string encode_seg_image_text(const uint8_t *buf, size_t len)
{
    ...
    std::ostringstream oss;
    oss << "CROW_CT_SEGIMG magic=0x" << to_hex(reinterpret_cast<const uint8_t *>(&magic), 4);
    oss << " seg_idx=" << seg_idx;
