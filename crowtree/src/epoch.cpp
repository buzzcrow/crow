...
EpochManager::Participant* EpochManager::participant_for_this_thread() {
  ...
  {
    p->next = head;
  } while (!participants_.compare_exchange_weak(head, p, std::memory_order_release,
...
void EpochManager::retire(void* ptr, Deleter deleter) {
  ...
  retired_.push_back(Retired{global_epoch_.load(std::memory_order_relaxed), ptr, std::move(deleter)});
