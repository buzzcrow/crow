...
bool logging_enabled() { return g_enabled.load(std::memory_order_relaxed); }
...
void init_logging(const std::string& log_dir, const std::string& level, size_t max_file_mb,
  ...
  try
  {
    ...
    auto sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(
        path, max_file_mb * 1024 * 1024, max_files);
    g_logger = std::make_shared<spdlog::async_logger>("crowtree", sink, g_tp,
...
bool logging_enabled() { return false; }
void shutdown_logging() {}
void init_logging(const std::string&, const std::string&, size_t, size_t) {}
