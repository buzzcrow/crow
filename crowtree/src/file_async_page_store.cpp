...
uint64_t BlockAsyncPageStore::submit_write(PageAddr, const void *, size_t, std::function<void(Status)> on_complete)
{
    if (on_complete) {
        on_complete(Status::invalid_argument("BlockAsyncPageStore: writes not supported"));
    }
    return 0;
}
...
{
    if (on_complete) {
        on_complete(Status::invalid_argument("BlockAsyncPageStore: fsync not supported"));
    }
    return Status::invalid_argument("BlockAsyncPageStore: fsync not supported");
}
