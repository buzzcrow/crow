...
inline uint64_t superblock_slot_bytes(uint32_t iu) { return round_up_to_iu(kSuperblockBytes, iu); }
inline uint64_t region_base_for(uint32_t iu) { return superblock_slot_bytes(iu) * 2; }
...
bool collect_live_extents(const PageStore& store, const Superblock& sb, uint32_t iu,
  ...
  for (uint64_t i = 0; i < count; ++i)
  {
    ...
    out->push_back({addr, round_up_to_iu(plen, iu)});
  }
  out->push_back({sb.manifest_addr, round_up_to_iu(sb.manifest_len, iu)});
  return true;
}

// build the allocator: free = the complement of `live` within
// [kRegionBase, file_size); append grows past EOF.
SpaceAllocator build_allocator(std::vector<std::pair<uint64_t, uint64_t>> live, uint64_t file_size,
                               uint32_t iu, uint64_t region_base) {
  SpaceAllocator a;
  a.iu = iu;
  ...
  for (const auto& e : live)
  {
    if (e.first > prev_end)
    {
      a.gaps.push_back({prev_end, e.first - prev_end});
    }
  ...
  if (eof > prev_end)
  {
    a.gaps.push_back({prev_end, eof - prev_end});  // dead tail
  }
  a.append = eof;
  return a;
}
...
Status Crowtree::checkpoint(uint64_t* out_last_applied) {
  ...
  // exact span.
  auto persist_one = [&](uint64_t the_page_id, PageBase* pg, const uint8_t* frame,
                         uint32_t plen) -> Status {
    if (pg->durable_addr == kNoAddr)
    {  // dirty: persist the live frame
      std::vector<uint8_t> blob;
      encode_durable_page(frame, plen, opt_.compression, &blob);
      uint32_t logical = static_cast<uint32_t>(blob.size());
  ...
  std::function<Status(uint64_t)> walk = [&](uint64_t page_id) -> Status {
    ...
    if (head->type == page_type::kBatchDelta)
    {
      ...
      LeafBase* fresh = build_leaf_spilling_locked(
          resolve_leaf_chain_for_rebuild(head, gc, &dead_overflow), right);
    ...
    const uint8_t* frame = base->type == page_type::kLeafBase
                               ? static_cast<LeafBase*>(base)->frame()
                               : static_cast<InnerBase*>(base)->frame();
    uint32_t plen = base->type == page_type::kLeafBase
    ...
    {  // leaf: persist its overflow chains too (reachable via cells, PT11)
  ...
  uint64_t sb_slot = (seq & 1) ? 0 : sb_slot_bytes;  // alternate A/B by parity
