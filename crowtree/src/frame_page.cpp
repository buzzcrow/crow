...
void frame_restamp_crc(uint8_t* f, uint32_t page_bytes) { stamp_trailer(f, page_bytes); }
...
bool leaf_frame_append_deltas(const uint8_t* src, uint32_t page_bytes,
  ...
  uint32_t slot_end = free_lo + (delta_count + static_cast<uint32_t>(entries.size())) *
                                    static_cast<uint32_t>(kLeafSlotSize);
  ...
  for (size_t j = 0; j < entries.size(); ++j)
  {
    ...
    uint32_t reclen = static_cast<uint32_t>(e.key.size() + e.cell.size());
    uint32_t rec_off = cur_hi - reclen;
    std::memcpy(out + rec_off, e.key.data(), e.key.size());
    std::memcpy(out + rec_off + e.key.size(), e.cell.data(), e.cell.size());
