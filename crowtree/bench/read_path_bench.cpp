...
//
// Build:
//   pixi run -- cmake -S crowtree -B crowtree/build-bench -DCROWTREE_BENCH=ON \
//     -DCMAKE_BUILD_TYPE=Release
//   cmake --build crowtree/build-bench -j
//   ./crowtree/build-bench/crowtree_bench --benchmark_filter=ReadPath
...
static void BM_ReadPath_Scan(benchmark::State &state)
{
    ...
    for (auto _ : state) {
        ...
        t->scan(Slice(), 1000000, &out, &truncated);
