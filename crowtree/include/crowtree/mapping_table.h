...
class MappingTable
{
  public:
    ...
    [[nodiscard]] static bool is_unloaded(PageBase *v)
    {
        return (reinterpret_cast<uintptr_t>(v) & kUnloadedBit) != 0;
    }

    [[nodiscard]] static PageBase *tag_unloaded(unloaded_page *u)
    {
        return reinterpret_cast<PageBase *>(reinterpret_cast<uintptr_t>(u) |
                                            kUnloadedBit); // NOLINT(performance-no-int-to-ptr)
    }

    [[nodiscard]] static unloaded_page *as_unloaded(PageBase *v)
    {
        return reinterpret_cast<unloaded_page *>(reinterpret_cast<uintptr_t>(v) &
                                                 ~kUnloadedBit); // NOLINT(performance-no-int-to-ptr)
    }
