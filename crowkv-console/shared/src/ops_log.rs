...
//! `~/.crowkv/log/`. Every outbound action issued by `shared::clients`
...
//! path derivation per design `~/.crowkv/log/console-<role>-<ts>-<pid>.log`,
...
/// `~/.crowkv/log/console-<role>-<unix_secs>-<pid>.log`. Errors are
...
mod tests {
    ...
    #[test]
    fn default_path_is_under_crowkv_log() {
        let p = default_path("test");
        let s = p.to_string_lossy().to_string();
        assert!(s.contains(".crowkv"), "{s}");
