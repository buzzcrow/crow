...
//! `x-crowkv-corr-id` header so a single user action shows up linked
//! across console + every upstream `crowkv-server` it touched.
//!
...
/// components and upstream `crowkv-server`. Lowercase per RFC 7230;
/// `crowkv-server` will recognise it once the tracing middleware lands.
pub const HEADER: &str = "x-crowkv-corr-id";
