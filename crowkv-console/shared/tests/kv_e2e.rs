...
async fn put_get_delete_cycle() {
    ...
    let _ = kv.put(group_id, b"alpha/1", b"a1", 0, 0).await.expect("seed alpha/1");
    let _ = kv.put(group_id, b"alpha/2", b"a2", 0, 0).await.expect("seed alpha/2");
    let _ = kv.put(group_id, b"beta/1", b"b1", 0, 0).await.expect("seed beta/1");
    ...
    let _ = kv2.put(group_id, b"cached", b"hit", 0, 0).await.expect("put on cached client");
