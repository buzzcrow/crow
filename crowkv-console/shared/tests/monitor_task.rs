...
use crowkv_console_shared::monitor::{spawn, MonitorConfig, ProbeTarget};
use serde_json::json;
...
async fn start_fake(node_id: &str, store_id: u64) -> FakeServer {
    ...
    FakeServer { addr, shutdown: tx, handle }
}
...
async fn monitor_tracks_three_nodes_and_marks_one_down() {
    ...
    .await;
    assert!(all_up, "expected all three nodes up; cache: {:?}", cache.snapshot().await);
    ...
    .await;
    assert!(n2_down, "n2 never marked Down; cache: {:?}", cache.snapshot().await);
...
async fn invalidate_triggers_out_of_cycle_refresh() {
    ...
    let up = wait_for(Duration::from_secs(3), move || {
        let c = Arc::clone(&c);
        Box::pin(async move { c.snapshot().await.get("n1").is_some_and(|r| r.health == NodeHealth::Up) })
    })
    .await;
    assert!(up);
