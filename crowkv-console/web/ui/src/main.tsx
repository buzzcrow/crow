...
import App, { type CrowkvConsoleProps } from "./App";
 ...
 * This mirrors `CrowkvConsoleProps` so a reverse-proxied or embedded
 * deployment can be configured without a rebuild:
 *   ?apiPrefix=/proxy/api&readonly=1&disableModules=kv,swagger&view=Physical
 */
function propsFromQuery(search: string): CrowkvConsoleProps {
  const q = new URLSearchParams(search);
  const props: CrowkvConsoleProps = {};
