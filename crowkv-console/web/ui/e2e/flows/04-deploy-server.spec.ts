...
test.describe('E2E-04 deploy server', () => {
  test('deploys and stops a real crowkv-server through the UI', async ({ page, baseURL }) => {
    ...
    try {
      ...
      await expect(page.getByRole('treeitem').filter({ hasText: 'n4' })).toBeVisible({ timeout: 15_000 });

      await page.getByRole('treeitem').filter({ hasText: 'n4' }).click({ button: 'right' });
      await page.getByRole('menuitem', { name: /deploy server/i }).click();

      await expect(page.getByRole('dialog', { name: /deploy server on n4/i })).toBeVisible();
      ...
      await expect(page.getByText(/Server deployed on n4/)).toBeVisible({ timeout: 30_000 });
