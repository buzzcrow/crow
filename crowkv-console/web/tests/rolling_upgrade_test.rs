...
//! does not diverge.
//!
//! By default the test uses the current `crowkv-server` binary for all
...
//! set `CROWKV_SERVER_BIN_V2` to a different binary (e.g. an older build).
//!
...
use crowkv_console_shared::lifecycle::{self, crowkv_server_bin, process_is_alive, DeployRequest};
...
fn resolve_second_binary() -> Option<PathBuf> {
    if let Ok(v2) = std::env::var("CROWKV_SERVER_BIN_V2") {
...
async fn mixed_version_3_node_cluster_kv_no_divergence() {
    ...
    // For a real version boundary, set CROWKV_SERVER_BIN_V2 to an old build.
    let v2_binary = resolve_second_binary().unwrap_or_else(|| {
        let copy = current.parent().unwrap().join("crowkv-server-v2");
