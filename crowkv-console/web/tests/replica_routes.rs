...
//! (matches the pattern in `mgmt_routes.rs` / `kv_routes.rs`).
...
async fn replica_add_remove_wires_peers_bidirectionally() {
    ...
    let r1_remotes = c1.list_remotes(sid, gid).await.unwrap();
    let r2_remotes = c2.list_remotes(sid, gid).await.unwrap();
    assert!(r1_remotes.iter().any(|r| r.replica_id == rid2), "n1 should list r2 as remote: {r1_remotes:?}");
    assert!(r2_remotes.iter().any(|r| r.replica_id == rid1), "n2 should list r1 as remote: {r2_remotes:?}");

    // 6. GET single replica detail.
    let resp = http.get(format!("{base}/api/stores/{sid}/groups/{gid}/replicas/{rid2}")).send().await.unwrap();
    ...
    let resp = http.delete(format!("{base}/api/stores/{sid}/groups/{gid}/replicas/999")).send().await.unwrap();
    assert_eq!(resp.status(), 404);

    // 8. DELETE replica 2 → orchestrated remove (deregister from n1,
    //    delete local on n2).
    let resp = http.delete(format!("{base}/api/stores/{sid}/groups/{gid}/replicas/{rid2}")).send().await.unwrap();
    ...
    let r1_remotes_after = c1.list_remotes(sid, gid).await.unwrap();
    assert!(
