...
async fn root_serves_spa_or_fallback() {
    ...
    if dist_index.is_file() {
        ...
        assert!(body.contains("id=\"root\""), "built SPA missing #root mount: {body}");
    } else {
        // Fallback path: must contain the instructional message.
        assert!(body.contains("UI not built"), "fallback page missing marker: {body}");
...
async fn deep_link_falls_through_to_index() {
    ...
    let addr = spawn_server().await;
    let resp = reqwest::get(format!("http://{addr}/some/spa/route")).await.expect("GET deep");
...
async fn api_routes_are_not_shadowed_by_spa_fallback() {
    ...
    let body = reqwest::get(format!("http://{addr}/healthz")).await.expect("healthz").text().await.expect("text");
    assert_eq!(body, "ok");
    ...
    let resp = reqwest::get(format!("http://{addr}/api/racks")).await.expect("racks");
    assert!(resp.status().is_success());
    let ct = resp
        .headers()
        ...
        .unwrap_or_default();
    assert!(ct.starts_with("application/json"), "expected JSON, got Content-Type={ct}");
}
...
async fn path_traversal_is_rejected() {
    ...
    let resp = client.get(format!("http://{addr}/../Cargo.toml")).send().await.expect("traversal");
    ...
    assert!(!body.contains("[package]") && !body.contains("[workspace]"), "leaked Cargo.toml contents: {body}");
}
