...
async fn rack_node_crud_through_web_routes() {
    ...
    // Add rack.
    let (s, v) = json_post(&client, &format!("{base}/api/racks"), json!({ "id": "r1", "name": "rack-1" })).await;
    ...
    assert!(!on_disk.contains("[[rack]]"), "rack should be gone from {cfg_path:?}: {on_disk}");
...
async fn deploy_then_restart_local_server() {
    ...
    assert_eq!(v["mgmt_url"], format!("http://127.0.0.1:{mgmt_port}"), "restart reuses recorded ports");
    ...
    let _ = std::process::Command::new("kill").arg("-KILL").arg(old_pid.to_string()).status();
    let _ = std::process::Command::new("kill").arg("-KILL").arg(new_pid.to_string()).status();
}
