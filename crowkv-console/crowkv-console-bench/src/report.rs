...
use crate::workload::{OpKind, WorkloadKind};
...
impl OpStats {
    /// Build an empty `OpStats` with a histogram bounded to 60 seconds
    /// at 3 significant digits — overkill for healthy local benches,
    /// safe under pathological tail latencies.
    ///
    /// # Panics
    /// Panics if `Histogram::new_with_bounds` rejects the constants
    /// (it shouldn't with these literals).
    #[must_use]
    pub fn new() -> Self {
        Self {
            ops: 0,
            errors: 0,
            not_found: 0,
            histogram: Histogram::new_with_bounds(1, 60_000_000, 3).expect("hdr histogram bounds"),
        }
    }
    ...
    pub fn record(&mut self, latency_us: u64, ok: bool, not_found: bool) {
        ...
        // saturate to histogram max instead of erroring out.
        let v = latency_us.clamp(1, self.histogram.high());
