...
impl ConsoleConfig {
    /// Default path: `$CROWKV_CONSOLE_CONFIG` if set, else
    /// `~/.crowkv/console.toml`. Returns `None` if no home dir is found
    /// and the env var is unset.
    #[must_use]
    pub fn default_path() -> Option<PathBuf> {
        if let Ok(p) = std::env::var("CROWKV_CONSOLE_CONFIG") {
            return Some(PathBuf::from(p));
        }
        let home = dirs::home_dir()?;
        Some(home.join(".crowkv").join("console.toml"))
    }
