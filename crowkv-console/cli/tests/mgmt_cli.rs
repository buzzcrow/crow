...
async fn store_paxos_replica_round_trip() {
    ...
    let (code, stdout, _) = run(&cli, &console_url, &["paxos", "inspect", "--store-id", store_id, "--group-id", group_id_2]);
    assert_eq!(code, 0);
    assert!(stdout.contains("n1"), "stdout={stdout}");

    // paxos remove
    let (code, _, stderr) = run(&cli, &console_url, &["paxos", "remove", "--store-id", store_id, "--group-id", group_id_2]);
    assert_eq!(code, 0, "stderr={stderr}");
