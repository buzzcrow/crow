...
fn run(cli: &PathBuf, console_url: &str, args: &[&str]) -> (i32, String, String) {
    let out = Command::new(cli).arg("--console").arg(console_url).args(args).output().expect("spawn cli");
    (
...
async fn kv_put_get_delete_round_trip() {
    ...
    let (code, stdout, stderr) = run(
        &cli,
        &console_url,
        &["kv", "put", "--store-id", "1", "--group-id", "1", "--key", "color", "--value", "indigo"],
    );
    assert_eq!(code, 0, "stdout={stdout}\nstderr={stderr}");
    assert!(stdout.contains("ok:"));

    // get
    let (code, stdout, _) = run(&cli, &console_url, &["kv", "get", "--store-id", "1", "--group-id", "1", "--key", "color"]);
    assert_eq!(code, 0);
    assert!(stdout.contains("indigo"), "stdout={stdout}");

    // delete
    let (code, _, stderr) = run(&cli, &console_url, &["kv", "delete", "--store-id", "1", "--group-id", "1", "--key", "color"]);
    ...
    let _ = run(&cli, &console_url, &["kv", "put", "--store-id", "1", "--group-id", "1", "--key", "scan/a", "--value", "1"]);
    let _ = run(&cli, &console_url, &["kv", "put", "--store-id", "1", "--group-id", "1", "--key", "scan/b", "--value", "2"]);
    let (code, stdout, stderr) = run(&cli, &console_url, &["kv", "list", "--store-id", "1", "--group-id", "1", "--prefix", "scan/"]);
    assert_eq!(code, 0, "stderr={stderr}");
