...
/// that don't set it.
#[must_use]
pub fn crowkv_cli_bin() -> PathBuf {
    if let Some(path) = option_env!("CARGO_BIN_EXE_crowkv-cli") {
...
/// A real `crowkv-server` process forked on loopback.
pub struct Upstream {
    pub pid: u32,
    pub mgmt_url: String,
    pub grpc_url: String,
...
/// Fork a real `crowkv-server` for node `n1`. Returns `None` (so the
