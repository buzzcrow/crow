...
- Do not auto-commit after every interaction — only commit when a distinct task type is complete. Before committing, verify no temp/generated files are staged; add them to `.gitignore` if needed.
