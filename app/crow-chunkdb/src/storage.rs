...
//!
//! `ChunkStore` reads/writes `Chunk` proto records to CROW KV via the
...
use crow_kv_client::{CrowkvClient, GetOutcome, ReadMode, ScanOutcome};
...
/// Chunk metadata store — persists `Chunk` records to CROW KV.
...
impl ChunkStore {
    #[must_use]
    pub fn new(kv: Arc<CrowkvClient>, bindings: BindingCache) -> Self {
        Self { kv, bindings }
    }
