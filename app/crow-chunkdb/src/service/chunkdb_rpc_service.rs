...
//! crow-rpc handler set for `ChunkdbService` (R116 migration).
//!
...
/// crow-rpc handler set for `ChunkdbService`. Holds the same
...
fn parse_fb_chunk_strip(fb: &FBChunkStrip<'_>) -> Option<ChunkStrip> {
    use crow_protocol::chunkdb::rpc::{EcStrip, MirrorStrip};
