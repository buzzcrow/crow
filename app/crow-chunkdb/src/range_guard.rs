...
use crow_protocol::common::{ChunkId, ChunkdbRangeBindingValue};
...
impl RangeGuard {
    ...
    pub async fn load_from_group0(
        ...
        instance_id: u64,
    ) -> crow_kv_client::Result<()> {
        use crow_kv_client::ReadMode;
        ...
        loop {
            ...
            for (k, v) in &outcome.items {
                let path = std::str::from_utf8(k).map_err(|e| crow_kv_client::Error::SysdataDecode {
                    key: prefix.clone(),
                    reason: e.to_string(),
                })?;
                let val: ChunkdbRangeBindingValue =
                    serde_json::from_slice(v).map_err(|e| crow_kv_client::Error::SysdataDecode {
                        key: path.to_string(),
                        reason: e.to_string(),
                    })?;
