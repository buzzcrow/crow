...
namespace
{
std::shared_ptr<crow::diskio::BlockingEngine> make_engine()
{
    return std::make_shared<crow::diskio::BlockingEngine>(2);
}

std::vector<crow::diskio::Zone> make_zones()
{
...
TEST(NullDisk, WriteReturnsSuccess)
{
    ...
    auto disk   = std::make_shared<crow::diskio::NullDisk>(crow::diskio::DiskId{1, 1}, engine, make_zones());
...
{
    auto engine = make_engine();
    auto disk   = std::make_shared<crow::diskio::NullDisk>(crow::diskio::DiskId{2, 2}, engine, make_zones());
...
{
    auto engine = make_engine();
    auto disk1  = std::make_shared<crow::diskio::NullDisk>(crow::diskio::DiskId{3, 3}, engine, make_zones());
    auto disk2  = std::make_shared<crow::diskio::NullDisk>(crow::diskio::DiskId{4, 4}, engine, make_zones());
    std::vector<uint8_t> out1(4096, 0);
    std::vector<uint8_t> out2(4096, 0);
...
{
    auto             engine = make_engine();
    auto             disk = std::make_shared<crow::diskio::NullDisk>(crow::diskio::DiskId{5, 5}, engine, make_zones());
    std::atomic<int> res{-1};
...
{
    auto engine = make_engine();
    auto disk   = std::make_shared<crow::diskio::MemDisk>(crow::diskio::DiskId{6, 6}, engine, make_zones());
...
{
    auto             engine = make_engine();
    auto             disk   = std::make_shared<crow::diskio::MemDisk>(crow::diskio::DiskId{7, 7}, engine, make_zones());
    std::atomic<int> res{-1};
    disk->engine()->submit_fsync(disk.get(), [&](int r) { res.store(r, std::memory_order_relaxed); });
    wait_for([&] { return res.load() != -1; });
    EXPECT_EQ(res.load(), 0);
...
{
    auto                         engine = make_engine();
    crow::diskio::DiskProperties props{0, 0, 1.0};
    auto disk = std::make_shared<crow::diskio::NullDisk>(crow::diskio::DiskId{8, 8}, engine, make_zones(), props);
    std::vector<uint8_t> data(4096, 0xAB);
...
{
    auto                         engine = make_engine();
    crow::diskio::DiskProperties props{0, 0, 0.0};
    auto disk = std::make_shared<crow::diskio::NullDisk>(crow::diskio::DiskId{9, 9}, engine, make_zones(), props);
    std::vector<uint8_t> data(4096, 0xAB);
    std::atomic<int>     res{-1};
    disk->engine()->submit_write(disk.get(), 0, data.data(), data.size(),
                                 [&](int r) { res.store(r, std::memory_order_relaxed); });
...
{
    auto                         engine = make_engine();
    crow::diskio::DiskProperties props{50, 50, 0.0};
    auto disk = std::make_shared<crow::diskio::NullDisk>(crow::diskio::DiskId{10, 10}, engine, make_zones(), props);
    std::vector<uint8_t> data(4096, 0xAB);
...
{
    auto                         engine = make_engine();
    crow::diskio::DiskProperties props{0, 0, 0.5};
    auto disk = std::make_shared<crow::diskio::NullDisk>(crow::diskio::DiskId{11, 11}, engine, make_zones(), props);
