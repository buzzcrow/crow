...
Buffer *build_write_request(BufferPool *pool, uint64_t req_id, crow::diskio::DiskId disk_id, uint32_t zone_index,
...
Buffer *build_read_request(BufferPool *pool, uint64_t req_id, crow::diskio::DiskId disk_id, uint32_t zone_index,
                           uint64_t zone_offset, uint32_t size)
{
    flatbuffers::FlatBufferBuilder fbb(128);
    crow::rpc::proto::FBInt128 fb_disk_id(disk_id.high, disk_id.low);
...
extern "C" void dio_on_complete(uint64_t request_id, crow_rpc_buffer_t control, crow_rpc_buffer_t data,
