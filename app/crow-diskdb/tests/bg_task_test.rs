...
use crow_diskdb::bg_task::{BackgroundTask, BgCtx, BgError, CycleFut, Trigger};
...
fn make_ctx() -> Arc<BgCtx> {
    ...
    let kv = Arc::new(DdbKvClient::new(crow_kv_client::CrowkvClient::new(
        crow_kv_client::ClientConfig::new(vec!["127.0.0.1:0".to_string()]),
    )));
...
async fn bg_runner_stops_all_tasks_on_stop_signal() {
    ...
    let mut runner = crow_diskdb::bg_task::BgRunner::new();
...
async fn bg_runner_empty_spawn_completes_immediately() {
    let ctx = make_ctx();
    let runner = crow_diskdb::bg_task::BgRunner::new();
    let stop = runner.stop_handle();
    let handles = runner.spawn(&ctx);
