...
//! The deploy path requires the `crow-kv-server` binary and skips
//! silently otherwise.
...
use crow_console_shared::lifecycle::{crow_kv_server_bin, stop_pid_with_timeout};
...
async fn multiple_racks_and_nodes_create_expected_workspaces() {
    let bin = match crow_kv_server_bin() {
    ...
    assert!(dir
        .join(format!("N-1/log/crow-kv-server-{pid1}.out.log"))
        .exists());
    assert!(std::fs::read_dir(dir.join("N-10/bin")).unwrap().next().is_some());
    assert!(dir
        .join(format!("N-10/log/crow-kv-server-{pid10}.out.log"))
        .exists());
