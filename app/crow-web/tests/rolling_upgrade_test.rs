...
//! two different `crow-kv-server` binary builds and verify a KV workload
//! does not diverge.
//!
//! By default the test uses the current `crow-kv-server` binary for all
...
//! set `CROW_KV_SERVER_BIN_V2` to a different binary (e.g. an older build).
//!
...
fn resolve_second_binary() -> Option<PathBuf> {
    if let Ok(v2) = std::env::var("CROW_KV_SERVER_BIN_V2") {
...
async fn mixed_version_3_node_cluster_kv_no_divergence() {
    ...
    // For a real version boundary, set CROW_KV_SERVER_BIN_V2 to an old build.
    let v2_binary = resolve_second_binary().unwrap_or_else(|| {
        let copy = current.parent().unwrap().join("crow-kv-server-v2");
