...
use crow_kv::kv::{CrowTreeBackend, CrowTreeEngine, CrowTreeOptions, KVEngine};
...
async fn open_crow_tree_engine(
    ...
    let opt = CrowTreeOptions {
        ...
        log_file_prefix: "crow-kv-server-tree".to_string(),
        ..Default::default()
    };
    info!(
        store_id,
    ...
    // asynchrony behind it (see `crow_kv::kv::CrowTreeEngine`'s docs). This
...
pub async fn create_group_with_wal(
    ...
    if std::env::var("CROW_KV_WAL_TEXT").as_deref() == Ok("1") {
        wal_config.wal_record_format = crow_kv::wal::WalRecordFormat::TextLine;
    }
