...
use bytes::Bytes;
use crow_kv::cluster::group::ProposeResult;
use crow_kv::cluster::group_election::LeaderElection;
use crow_kv::cluster::local_replica::PxLocalReplicaRole;
use crow_kv::common::config::{CrowKVConfig, WalConfig};
use crow_kv::kv::CrowTreeBackend;
...
use crow_kv_server::startup::{create_group_with_wal, store_wal_root};
...
async fn create_group_with_wal_restores_and_resumes_at_next_slot() {
    ...
    assert!(replay.records.iter().any(|record| {
        record.slot == 3 && matches!(record.record_type, crow_kv::wal::record::RecordType::Accepted)
    }));
}
...
async fn crow_tree_engine_persists_across_restart(crowtree_backend: CrowTreeBackend) {
    ...
    let ct_path = crow_kv_server::startup::store_crow_tree_path(&data_root, store_id, group_id);
    assert!(
        ct_path.exists(),
