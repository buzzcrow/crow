...
async fn scan_zone_records(
    ...
    Option<crowdb_protocol::diskdb::rpc::ZoneValue>,
    Vec<Vec<u8>>,
    Vec<Vec<u8>>,
) {
    let (store_id, group_id) = bind;
    let mut zone_value: Option<crowdb_protocol::diskdb::rpc::ZoneValue> = None;
    let mut corrupt_busy: Vec<Vec<u8>> = Vec::new();
    let mut corrupt_free: Vec<Vec<u8>> = Vec::new();

    // ZoneValue.
    let zone_key = crowdb_protocol::key::ZoneKey { disk_id, zone_index };
    let zone_bytes = zone_key.to_bytes();
    if let Ok(crowdb_kv_client::GetOutcome::Found { value, .. }) = kv
        .kv()
        .get(
            store_id,
            group_id,
        ...
        .await
    {
        ...
        if let Ok(zv) = crowdb_protocol::diskdb::rpc::ZoneValue::from_bytes(&value) {
            ...
            zone_value = Some(crowdb_protocol::diskdb::rpc::ZoneValue {
