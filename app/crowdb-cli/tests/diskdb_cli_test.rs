...
//! `crowdb-diskdb` binary and skips silently if not built.

mod common;

use std::time::Duration;

use common::console::{crowdb_cli_bin, pick_free_port_range, run, spawn_console_empty};
...
async fn diskdb_cli_deploy_restart_stop_delete_lifecycle() {
    ...
    let Some(diskdb_bin) = crowdb_diskdb_bin().filter(|p| p.exists()) else {
        eprintln!("skipping: crowdb-diskdb binary not built");
        return;
    };
    let diskdb_bin = diskdb_bin.to_string_lossy().into_owned();
    // The deploy handler finds the binary via crowdb_diskdb_bin() which
    // searches next to the console's exe. Set CROWDB_DISKDB_BIN so the
    // in-process console can find it.
    std::env::set_var("CROWDB_DISKDB_BIN", &diskdb_bin);
    ...
    std::env::remove_var("CROWDB_DISKDB_BIN");
    tokio::time::sleep(Duration::from_millis(50)).await;
    let _ = std::fs::remove_dir_all(dir);
}
