...
//!
//! Key work: percentile extraction from `crowdb_common::metrics::PreciseHistogram`,
...
use crowdb_common::metrics::{PreciseHistogram, SystemMetrics};
use serde::{Deserialize, Serialize};

use super::workload::{OpKind, WorkloadKind};
use crowdb_kv_client::ClientMetricsSnapshot;
...
pub(crate) struct ServerMetrics {
    ...
    /// System resource usage (see `crowdb_kv::metrics::system`).
    pub(crate) system: SystemMetrics,
    /// Server-side crowdb-rpc transport stats (aggregated across nodes):
...
/// crowdb-rpc transport stats: submit→writev queue-wait latency.
