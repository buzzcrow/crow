...
impl BenchReport {
    ...
    pub(crate) fn markdown_report(
        ...
        let _ = writeln!(out, "# CrowDB Benchmark Report");
        let _ = writeln!(out);
        ...
        let _ = writeln!(out, "- **connections:** {} (crowdb-rpc channels)", self.connections);
        let _ = writeln!(
            out,
        ...
        let (wal_desc, kv_desc) = match self.mode.as_str() {
            "mem" => (
                ...
                "crowdb-tree + mem-block page store (in-memory, no disk I/O)",
            ),
            "file" => (
                "file (file-backed, no O_DIRECT)",
                "crowdb-tree + file page store (file-backed, no O_DIRECT)",
            ),
            "block-device" => (
                "block-device (O_DIRECT, 4K aligned)",
                "crowdb-tree + block page store (O_DIRECT, 4K aligned)",
            ),
            other => (other, other),
        };
