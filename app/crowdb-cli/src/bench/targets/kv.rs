...
//! (`crowdb-console/web/ui/e2e/fixtures/consoleSetup.ts`), but through
//! the typed `ConsoleClient` against an **embedded** `crowdb-web`
...
use crowdb_console_shared::clients::console::{
...
use crowdb_console_shared::test_ports::unique_test_port;
...
pub enum BenchMode {
    /// Crowdbtree engine + mem-block page store (in-memory, no alignment),
    /// WAL on mem-block backend.
    Memory,
    /// Crowdbtree engine + file page store (file-based, no alignment),
    /// WAL on file backend.
    File,
    /// Crowdbtree engine + block page store (`O_DIRECT`, 4K aligned),
...
/// `crowdb-web` console instance. Encapsulates the full deploy
...
impl BenchFixture {
    ...
    /// create 1 rack + 3 nodes, deploy a `crowdb-kv-server` per node in
    ...
    pub async fn new(
        ...
        let state = crowdb_web::AppState::with_config(ConsoleConfig::default(), Some(console_config_path));
        let console_task = tokio::spawn(async move {
            let _ = axum::serve(listener, crowdb_web::router(state)).await;
        });
    ...
    /// `crowdb-kv-server` on each, in `mode`. Returns the node ids and their
    ...
    /// The elected leader's crowdb-rpc endpoint, ready to hand to
    ...
    /// Per-node `crowdb-kv-server` management-API URLs (one per deployed
    ...
    /// Build a map from crowdb-rpc endpoint URL to node ID, for resolving
    ...
    /// Locate and read this node's `log/crowdb-kv-server-metrics-<timestamp>-<pid>.log`
    /// file (see `crowdb_kv::common::logging::open_metrics_log`).
