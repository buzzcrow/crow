...
//! `crowdb-rpc-fb-server` (separate process, separate epoll fd), then
...
use crowdb_rpc_ffi::sys;
use crowdb_rpc_ffi::{Buffer, BufferPool, Connection, CrowdbRpcLatencyStats, RpcClient, RpcServer};
...
impl BenchTarget for RpcTarget {
    ...
    async fn provision(&mut self, cfg: &BenchConfig) -> Result<()> {
        ...
        // `crowdb-rpc-fb-server --port=18080 --io_workers=2`).
        self.port = cfg.server_port.ok_or_else(|| {
            Error::Config("fb server port is required (start crowdb-rpc-fb-server first)".to_string())
        })?;
        ...
        if let Some(ref log_dir) = self.log_dir {
            ...
            crowdb_rpc_ffi::logging::init_logging(log_dir, "info", 30, 5, "bench-client");
    ...
    async fn cleanup(&mut self) {
        ...
        if self.log_dir.is_some() {
            crowdb_rpc_ffi::logging::shutdown_logging();
        }
...
impl RpcTarget {
    ...
    fn run_cpp_coroutine_workers(
        ...
        let handle = tokio::task::spawn_blocking(move || {
            ...
            let conn_handles: Vec<sys::crowdb_rpc_conn_t> =
                conn_usizes.iter().map(|u| *u as sys::crowdb_rpc_conn_t).collect();
            let conn_ptr = conn_handles.as_ptr();

            let client_handle = client_handle_usize as sys::crowdb_rpc_client_t;
            let server_handle = server_handle_usize as sys::crowdb_rpc_server_t;
            ...
            let mut cc = sys::CrowdbRpcClientCounters::default();
            unsafe { sys::crowdb_rpc_client_get_counters(client_handle, &mut cc) };
            eprintln!(
...
unsafe extern "C" fn co_on_response(
    ...
    let ok = status == sys::CROWDB_RPC_OK;
