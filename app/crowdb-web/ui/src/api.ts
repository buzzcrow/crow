...
// Thin typed wrappers over the crowdb-web HTTP API.
 ...
 * `crowdb-console-shared::config`). The tagged `SshCreds` enum is a
 ...
 * `CrowdbConsoleProps.apiPrefix`) re-root all data-plane traffic under a
...
/**
 * Deploy a crowdb-kv-server instance to a node
 */
...
/**
 * Get the OpenAPI spec for a node's crowdb-kv-server instance
 */
...
/**
 * Body accepted by `POST /api/stores` (`crowdb_web::mgmt::CreateStoreBody`).
 ...
 * running `crowdb-kv-server` (deployed via `deployServer`).
 */
 ...
 * (`crowdb_web::mgmt::ClusterInitBody`).
 */
