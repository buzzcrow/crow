...
//! CLI and web UI requests through here → `DiskdbClient` → crowdb-rpc →
//! `crowdb-diskdb`. No direct crowdb-rpc from the browser or CLI.
...
use crowdb_protocol::common::{DiskGroupUsageSummary, DiskdbExtra, HwStatus, InstanceValue};
...
fn disk_id_to_string(d: &crowdb_protocol::common::DiskId) -> String {
...
///
/// # Errors
/// Returns `502` on crowdb-rpc or registry errors, `400` on invalid params.
...
pub async fn http_hardware_capacity(
    State(state): State<AppState>,
) -> Result<Json<crowdb_kv_client::HardwareCapacitySummary>, (StatusCode, Json<ErrorBody>)> {
    let hw = build_hardware_client(&state)
        .await
        .ok_or_else(|| err_502("no group-0 endpoint; cluster not initialized"))?;
    hw.capacity_summary()
...
fn parse_disk_id(s: &str) -> Option<crowdb_protocol::common::DiskId> {
    use crowdb_protocol::diskdb_type_util::DiskIdExt;
    crowdb_protocol::common::DiskId::from_display_string(s).ok()
}

/// `GET /api/diskdb/scan-status?dg=<id>` — `GetScanStatus`.
///
/// # Errors
/// Returns `502` on crowdb-rpc errors.
...
///
/// # Errors
/// Returns `400` if `disk_id` is missing, `502` on crowdb-rpc errors.
...
/// for all zones. Calls crowdb-rpc rebuild once per zone (or once with
/// `u32::MAX` for all).
///
/// # Errors
/// Returns `400` if `disk_id` is missing, `502` on crowdb-rpc errors.
...
impl From<crowdb_protocol::diskdb::rpc::ScanSummary> for ScanSummaryDto {
    fn from(s: crowdb_protocol::diskdb::rpc::ScanSummary) -> Self {
        Self {
...
impl From<crowdb_protocol::diskdb::rpc::GetScanStatusResponse> for ScanStatusResponse {
    fn from(r: crowdb_protocol::diskdb::rpc::GetScanStatusResponse) -> Self {
        Self {
            summary: r.summary.map(ScanSummaryDto::from),
            has_run: r.has_run,
