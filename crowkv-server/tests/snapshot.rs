//! Topology snapshot tests for `PxKvStore::snapshot()`. Covers composition
//! of per-layer snapshots, cheap kv-store stats, and per-remote metrics surface.
...
#[test]
fn snapshot_single_group_no_remotes() {
    let store = Arc::new(PxKvStore::new(1, "127.0.0.1:0".parse().unwrap()));
    store.add_group(PxGroup::new(7, PxLocalReplica::new(3, PxLocalReplicaRole::Leader)));
    let snap = store.snapshot();
...
#[test]
fn snapshot_with_remote_zero_metrics() {
    let store = Arc::new(PxKvStore::new(1, "127.0.0.1:0".parse().unwrap()));
