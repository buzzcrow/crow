...
//! (`graceful_shutdown` / management API) is covered by `management_api.rs`.
...
async fn shutdown_cascade_clean() {
    ...
    assert!(store.start().await, "start failed");
    assert!(store.listen_addr().is_some());

    let report = store.shutdown(Duration::from_secs(2)).await;
    assert!(report.is_clean(), "expected clean shutdown, got errors: {:?}", report.errors);
}
...
async fn shutdown_is_idempotent() {
    ...
    assert!(second.is_clean(), "second shutdown must be a clean no-op, got: {:?}", second.errors);
}
