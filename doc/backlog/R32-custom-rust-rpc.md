...
independently — length-prefixed framing is stateless, there is no shared
encoder table, no per-stream state, no flow-control windows. The kernel
TCP lock (fast spinlock, held for one syscall) is the only serialization
point. N threads produce N independent `write()` calls that the kernel
coalesces into one segment. The userspace funnel is gone.
...
- 2T:1C read throughput recovers the ~17% h2-lock loss; higher
  thread:connection ratios scale linearly with threads (no userspace
  funnel).
...
`doc/working/read-flow-analysis.md` ≈L520-564. This backlog item is the
