...
> **Model B, not Model A (2026-07):** M3 as originally written below assumed a self-hosted `Group-0` system group + `DescribeCluster` RPC + key-hash routing ("Model A"). That was never built. Verified-against-code gap analysis in [`plan-client.md`](plan-client.md) confirms M1/M2/M4 are done (ahead of schedule, with naming deviations noted there) under a different, already-shipped model ("Model B": operator/HTTP-managed topology, explicit `group_id` on every RPC). M3 is retargeted to that model; see `plan-client.md` for the authoritative C1-C3 milestone breakdown (topology cache, retry, read-mode client API) and the `crowkv-client` crate. The table below is kept for history; do not re-litigate Model A.
...
| **M3** | Client library, Model B scope (see `plan-client.md` §5): standalone `crowkv-client` crate, topology cache seeded from HTTP `/topology` (no `DescribeCluster`), retry policy reusing `(client_id, seq)` across retries, `ReadMode` passthrough including `ReadYourWrites` client-side tracking. | Per `plan-client.md` C1-C3 acceptance criteria. |
| **M4** | Read mode routing: `Linearizable` → leader only, lease check enforced (lease state machine unchanged from P1 M5, only clock source swaps `TestTimer` → real monotonic). `ReadYourWrites`/`BoundedStale`/`BestEffort` → any replica with sufficient resolved-slot. Lease fallback: ReadIndex as quorum heartbeat round-trip. | Mixed workload (50% leader reads, 50% follower reads) returns zero divergence; ReadIndex fallback exercised by disabling lease in test config. **Done** (server-side; `plan-client.md` C3 covers exposing all four modes on the client). |
...
| Milestone | Scope | Acceptance |
|---|---|---|
| **M1** | Snapshot install: wire `SnapshotService` to `Engine::snapshot_export`/`snapshot_import`. Resumable chunked transfer with `(snapshot_id, chunk_offset)` snapshoting; restart-after-failure resumes at last offset. End-to-end CRC before activation; throttle via `chunk_rate_bytes_per_sec`. New-node bootstrap: receive snapshot at slot S, catch up via WAL streaming `[S+1, current_max_chosen]`. | New empty node added to running 3-node group: snapshot installs, catches up, joins quorum, `compare()` equals existing learners. |
