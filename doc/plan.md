...
| **M2** | Minimal RPC service over loopback gRPC (`tonic` + `prost`). `.proto` schema for `Prepare`/`Promise`/`Accept`/`Accepted` with `version: u32` at tag 1. `Node` struct with `NodeRole` (hard-coded leader/follower). `TestNodeHarness` spawns nodes on `127.0.0.1:0`. `MinimalProposer` drives classic / optimized / multi-Paxos rounds. | Integration tests: S0-A classic (3-node), S0-B optimized (3-node), S0-C multi-Paxos 10 slots (5-node), S0-D quorum rejection (5-node). All pass. |
...
| **M4** | `Proposer`: slot counter (single async task), sliding window (default 16), bounded admission queue, `Busy` backpressure. `Replicator`: per-peer `Accept` fan-out via `mpsc`, per-slot quorum bitmap, per-peer flow control. Background `Repair` task: gap detection by age/count threshold, classic Paxos repair. | Unit tests: 10 parallel slots chosen out of order, gap repair fills missing slot, safe-slot advances. |
...
| **M6** | Per-`client_id` last-sequence map + last-result cache. `DedupCheckpoint` entry kind (stubbed, in-memory only). | Unit tests: retry same `(client_id, seq)` returns same result; different seq advances. |
