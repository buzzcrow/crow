...
Depends on: [`design.md`](design.md), [`requirement.md`](requirement.md), [`plan.md`](plan.md) §7 (Concurrency Model)
...
- Avoid blocking the consensus async tasks on disk syscalls, in line with [`plan.md`](plan.md) §7 rule 1.
...
Per [`plan.md`](plan.md) §7, every business-logic path in CrowKV is `async`. Disk syscalls (`fdatasync`, `pwrite`, `pread`) are blocking. Two options to bridge:
...
- Inject a deterministic in-memory backend for `test_harness`-driven unit tests (per [`test-design-wal.md`](test-design-wal.md) §5).
...
| Unit tests for WAL, engine | A third **simulated** backend (`SimDisk`) that holds an in-memory `BTreeMap<u64, Vec<u8>>` and exposes the same `AsyncFile` API | `test_harness` (per [`test-design-wal.md`](test-design-wal.md) §5) |
| Integration tests | Real backend on a `tempfile`-managed directory | `cargo test --test wal_integration` |

The simulated backend supports failure injection: `SimDisk::set_full()`, `SimDisk::inject_io_error()`, `SimDisk::corrupt_at_offset()` — these are the same hooks listed in [`test-design.md`](test-design.md) §4 and [`test-design-wal.md`](test-design-wal.md) §3.
...
- **Per-disk topology B**: revisit once we have multi-disk benchmark numbers from [`plan-wal.md`](plan-wal.md) M3.
