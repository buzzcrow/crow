...
**Landed so far** (Phase A, B, C1/C2/C3/C4/C5/C6/C7/C8/C9,
D1/D2/D3/D4/D5/D6/D7/D8/D9/D10/D11, E1/E3-E14/E15):
`ops::hardware` disk-group/disk CRUD + 16 unit tests,
...
optional workspace dir + optional PID override. mgmt store/group/replica/
init handlers migrated to `ops::kv_logical::*` / `ops::cluster::*`. KV
data-plane handlers migrated to `ops::kv_data::*`. `usePhysicalTree`
renamed to `useClusterTree` with disk-group/disk fetch. Sidebar
per-domain IA fixed (Cluster: rack→node→DG→disk; KV: rack→node→KV-server
→store→group→replica; Chunk: rack→node→diskdb→DG→disk). `Disk` node
...
- [x] **C4: Migrate `http_delete_node_server` + require-empty** —
  ...
  handling).
  Files: `app/crowdb-web/src/lifecycle.rs`.
- [x] **C6: Migrate mgmt store/group/replica/init handlers** —
...
- [x] **C7: Migrate KV data-plane handlers** — `http_kv_get`,
...
- [x] **C9: Integration tests for migrated handlers** — verify
...
- [x] **D3: `usePhysicalTree` → `useClusterTree`** — rename +
...
- [x] **D4: Sidebar per-domain tree (IA fix)** — current Sidebar
...
- [x] **D6: Topology canvas — Disk node type** — `DiskGroup` node
...
- [x] **D11: Vitest embedding-contract test** — `DomainContext`
...
- [x] **E3: `10-cluster-rack-node.spec.ts`** (renamed from
...
- [x] **E4: `11-cluster-server-lifecycle.spec.ts`** (renamed from
...
- [x] **E5: `12-cluster-node-inspect.spec.ts`** (renamed from
...
- [x] **E6: `20-kv-store-group.spec.ts`** (renamed from
...
- [x] **E7: `21-kv-reconfig.spec.ts`** (renamed from
...
- [x] **E10: `40-inspector-activity.spec.ts`** — domain-agnostic
...
- [x] **E14: `90-flow-full-chain.spec.ts`** — full chain across all
...
- [x] **F1: Run `pixi run -- cargo test -p crowdb-console-shared`** —
  all unit tests pass.
- [x] **F2: Run `pixi run -- cargo test -p crowdb-web`** —
  integration tests pass.
- [x] **F3: Run `pixi run -- cargo clippy --all-targets -- -D warnings`**
  — no warnings.
- [x] **F4: Run `pixi run -- cargo fmt --all -- --check`** — no
  formatting drift.
...
**Unit (Rust):**
- [x] `ops::kv_server::delete` require-empty conflict (via web
...
- [x] `POST /api/cluster/init` → `ops::cluster::init`.
...
- [x] `DELETE /api/nodes/:id/server` → `ops::kv_server::delete`
  (409 + 204) — require-empty check added; 409 + 204 integration test
  pending C9.
- [x] `POST /api/stores` → `ops::kv_logical::add_store`.
- [x] `GET /api/stores/:sid/groups/:gid/kv/get` → `ops::kv_data::get`.
- [x] `/api/swagger/` → 404.
- [x] `/api/nodes/:id/openapi.json` → 404.
...
- [x] `DELETE /api/nodes/:id/server` require-empty (409 + 204).
