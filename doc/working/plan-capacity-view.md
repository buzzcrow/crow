...
- [x] **1.7 REST proxy integration tests**: `/instances` returns registry data; `/usage` no-`dg` merges two instances; `/usage?dg=&disk=&zone=` returns `ZoneUsage` with `usage_bitmap`; scan/recalc/compact/rebuild proxy through; `PUT /api/disks/:id/status` writes + 404 unknown. Files: `app/crow-web/tests/diskdb_routes_test.rs`.
...
- [x] **1.10 Batch disk-add tests**: 3 valid → all created; 1 malformed → 0 created; duplicate ids → 400; group-0 write fails on disk 2 → rollback all. Files: `app/crow-web/tests/diskdb_routes_test.rs`.
...
- [x] **2.2 Console-shared unit tests**: mock HTTP server → each method deserializes correctly; error responses surface as typed errors. Files: `lib/crow-console-shared/tests/diskdb_client_test.rs`.
...
- [x] **2.4 CLI integration tests**: `crow diskdb status`/`usage`/`scan`/`scan-status`/`recalc`/`compact`/`rebuild` against a `crow-web` backed by `crow-diskdb`; unknown disk-id errors surface. Files: `app/crow-cli/tests/diskdb_cli_test.rs`.
...
- [x] **6.3 E2E: Capacity canvas**: DiskGroup per-disk boxes gradient; Disk zone grid 80×80; Zone bitmap 181×181 (busy red, free green, hover tooltip); 3s poll no flicker. Files: `app/crow-web/ui/e2e/flows/51-capacity-canvas.spec.ts`.
- [x] **6.4 E2E: ScannerPanel + RecalcPanel**: scan summary grouped; Run Scan triggers + shows running; recalc drift rows + Rebuild action. Files: `app/crow-web/ui/e2e/flows/51-capacity-canvas.spec.ts`.
