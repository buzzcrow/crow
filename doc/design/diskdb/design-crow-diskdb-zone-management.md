...
# CROW - Design: diskdb Zone Management

Depends on: [`design-crow-diskdb.md`](design-crow-diskdb.md) §3.3 (no CAS,
exclusive ownership), §3.9 (unit-based sizes, disk-id key routing), §6
(hierarchy);
[`design-crow-protocol-key.md`](../protocol/design-crow-protocol-key.md)
(binary key encoding);
[`design-crow-kv-state-machine.md`](../kv/design-crow-kv-state-machine.md)
(slot tracking, apply semantics).
Satisfies: `design-crow-diskdb.md` §7 (Zone Records and Crash Recovery),
...
(`doc/design/protocol/design-crow-protocol-key.md`): each is a flat
...
allocator is persistence (CROW writes `BusyBlockValue`/
...
3. CAS-set the bit via `compare_exchange` on the 64-bit word. On CAS
   ...
   **CAS retry bound:** per-bit CAS is capped at `cas_retry_limit`
   ...
   contention. The `cas_retry_count` atomic (and the wired crow-common
...
2. **Phase 2 (async)**: `.await` on crow-kv `batch_write` that Puts
...
1. **Scan free records** for the zone by key prefix (the free records
   of one zone are contiguous in the crow-tree page). This KV read is
...
6. **One atomic `batch_write`**: Put the new `ZoneValue` + Delete all
   ...
   `batch_write` on the bound data group — atomic via crow-kv paxos.
