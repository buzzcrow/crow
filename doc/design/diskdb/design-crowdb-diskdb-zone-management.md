...
```
# Busy block (on allocate; deleted by matching compaction)
BusyBlockKey { disk_id, zone_index, unit_offset }  -> BusyBlockValue

# Immutable free fact (on free; transient — deleted by compaction)
FreeBlockKey  { disk_id, zone_index, unit_offset, allocation_ts }
                                                       -> FreeBlockValue
...
`u32 BE`; `unit_offset` and `allocation_ts` are `u64 BE`. Big-endian fixed width gives
...
- **`BusyBlockValue`**:
  - `allocation_ts: u64` — monotonically increasing allocation incarnation,
    also returned in the `Segment`. It is an identity token, not a KV commit
    timestamp or a compaction-ordering value.
...
- **`FreeBlockValue`**:
  - `pre_allocation_ts: u64` — the exact busy incarnation released by this
    immutable free fact. It equals the `allocation_ts` in the request segment.
...
`min_slot` on the ordinary crowdb-kv scan request is a read-freshness floor,
not a record-slot filter. Compaction uses the R132 fixed-cutoff bounded scan,
which returns each current record's `commit_slot`. Strategy 2 uses
`JournalScan` for slot-ordered replay.
...
2. **Phase 2 (async)**: `.await` on crowdb-kv `batch_write` that Puts
   the `BusyBlockValue` with a new `allocation_ts` at `BusyBlockKey {
   disk_id, zone_idx, unit_offset }`. On success, return a `Segment`
   carrying the same incarnation. On failure:
...
**Why incarnation matching prevents stale free:**

Suppose allocation 1001 is freed and later the same physical range is
...
threshold), match free facts to busy incarnations, merge valid frees into the
`ZoneValue` bitmap, and delete the matched busy plus processed free records in
one `batch_write`. The fixed-cutoff bounded scan is the only KV extension.
...
- **Compaction crash safety** — the snapshot write and free-record
  ...
  (with `compact_slot` advanced) and the record deletions are
  durable, or neither is. There is no window where the snapshot is
  written but its corresponding deletions do not.
- **Re-allocate after compaction** — once compaction has cleared the
  bit and deleted the matching busy/free records, the block is available for
  re-allocation. The re-allocate is a normal allocate (CAS-set bit +
  persist `BusyBlockValue`) with a higher `allocation_ts`. A delayed old free
  creates only its old incarnation key; later compaction rejects the mismatch.
...
bitmap, with `snapshot_slot` (replay start point), `compact_slot`
(compaction watermark), and `crc32` (integrity check over
`usage_bitmap` + `compact_slot`). The baseline `ZoneValue` (empty bitmap,
`snapshot_slot = 0`, `compact_slot = 0`) is written during disk-add init
...
- `compact_zone_inner()` — bounded-read busy/free records (KV read, no lock),
  match allocation incarnations, acquire the zone lock, `range_clear` only
  matched free records, recompute `used_count`, release the lock, then write
  the new `ZoneValue` and exact deletions in one atomic `batch_write`. Used by `CompactionEngine`
...
`BusyBlockValue.allocation_ts` uses a per-disk-group monotonic source. The
source is an `AtomicU64` initialized above every durable busy and free
...
- **I10 — Persist-only recovery**: recovery replays busy records
  conservatively and leaves matching free facts for the common compaction
  flow. It never makes a range reusable from an unmatched or incomplete
  record view.
...
- `validate_owner_on_free` — removed; compaction validates the free fact
