...
# CROW - Design: diskdb Space Metrics + Query API

Depends on: [`design-crow-diskdb.md`](design-crow-diskdb.md) §3.4 (records are
source of truth), §7 (record model + recovery strategies), §3.7 (reuse
`crow-common` metrics), §8 (allocation algorithm); [`design-crow-kv-group0.md`](../kv/design-crow-kv-group0.md)
(hardware hierarchy + service registry).
Satisfies: `design-crow-diskdb.md` §11 (Space Metrics).
...
- `swap_periods` does `swap(0, Relaxed)` on each period counter; totals
  ...
  the crow-common `allocate_total`/`free_total` counters.
...
hierarchy) reusing `crow-common`'s `MetricsRegistry`. `DiskdbMetrics` in
...
bridged to the crow-common `Gauge`/`Counter` reporting layer on a
...
`SpaceUsageClient` in `lib/crow-kv-client/src/space_usage.rs` wraps
...
bytes + disk-group/disk counts. Mirror `crow-kv-client`'s retry pattern
...
- **I1 — Free = capacity − busy**: CROW reuses freed space immediately
