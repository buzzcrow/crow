...
# CROWDB - Design: chunkdb Range Binding + Instance Sharding

Depends on: [`design-crowdb-chunkdb.md`](design-crowdb-chunkdb.md) §3.6 (stateless),
§5.4a (hash bucket system), §5.4b (migration);
[`../kv/design-crowdb-kv-group0.md`](../kv/design-crowdb-kv-group0.md) §2.1 (sysdata API surface).
Satisfies: `design-crowdb-chunkdb.md` §5.4a (instance sharding by bucket range).
...
- [5. Dynamic Binding Monitor](#5-dynamic-binding-monitor)
  ...
  - [5.4 Monitor wiring in crowdb-kv-server](#54-monitor-wiring-in-crowdb-kv-server)
...
`lib/crowdb-protocol/src/key/chunkdb.rs` defines:
...
KV group routing (`BindingCache`). It lives in `crowdb-kv-client` (the
single sysdata API surface, group0 §2.1).

### 2.1 Struct + methods

`lib/crowdb-kv-client/src/range_binding.rs`:
...
- `from_shared(kv: Arc<CrowdbClient>) -> Self` — wrap an existing
  `CrowdbClient`.
...
The chunkdb client's `with_retry` loop (in `crowdb-chunkdb-client`) is
...
`crowdb-kv-client/src/config.rs`. `NotMyRange` is treated as transient
...
`app/crowdb-chunkdb/src/range_guard.rs`:
...
impl RangeGuard {
    ...
    pub async fn load_from_group0(&self, kv: &CrowdbClient, instance_id: u64) -> Result<()>;
}
```
...
`crowdb-kv-client`; the chunkdb-specific strategy lives alongside it.
The monitor is wired into `crowdb-kv-server`'s group-0 leader (not into
`crowdb-chunkdb`) because it writes to group-0 and needs service-registry
access. Both are group-0 concerns.

### 5.1 Common BindingStrategy trait

`lib/crowdb-kv-client/src/binding_framework.rs`:
...
pub trait BindingStrategy: Send + Sync {
    ...
    fn write_bindings(&self, kv: &CrowdbClient, bindings: &[Self::Binding]) -> impl Future<Output = Result<()>> + Send;
    fn read_bindings(&self, kv: &CrowdbClient) -> impl Future<Output = Result<Vec<Self::Binding>>> + Send;
}
```
...
`lib/crowdb-kv-client/src/chunkdb_binding_strategy.rs`:
...
### 5.4 Monitor wiring in crowdb-kv-server

`app/crowdb-kv-server/src/binding_monitor_wiring.rs`:
...
1. Build a `CrowdbClient` seeded with the group-0 leader endpoint.
...
`crowdb-kv-server`'s `main.rs` calls this after the keep-alive loop is
...
- Root design: [`design-crowdb-chunkdb.md`](design-crowdb-chunkdb.md)
  §3.6 (stateless), §5.4a (hash bucket system), §5.4b (migration).
- Group-0 sysdata API: [`../kv/design-crowdb-kv-group0.md`](../kv/design-crowdb-kv-group0.md) §2.1.
- `RangeBindingClient`: `lib/crowdb-kv-client/src/range_binding.rs`.
- `BindingStrategy` + `BindingMonitor`: `lib/crowdb-kv-client/src/binding_framework.rs`.
- `ChunkdbRangeStrategy`: `lib/crowdb-kv-client/src/chunkdb_binding_strategy.rs`.
- `RangeGuard`: `app/crowdb-chunkdb/src/range_guard.rs`.
- Monitor wiring: `app/crowdb-kv-server/src/binding_monitor_wiring.rs`.
- chunkdb keep-alive: `app/crowdb-chunkdb/src/main.rs` (`spawn_chunkdb_keepalive`).
- `NotMyRangeHint`: `lib/crowdb-protocol/src/fbs/chunkdb_type.fbs`.
- `ChunkdbRangeBindingValue` + `RangeStatus`: `lib/crowdb-protocol/src/fbs/sysdata_type.fbs`.
