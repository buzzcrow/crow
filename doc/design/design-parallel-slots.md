...
**Transport: per-peer bidi `PeerStream`.** The leader uses one long-running gRPC bidi stream per `(group_id, peer_id)` pair (see [`design-rpc.md`](design-rpc.md) §3). This stream carries all steady-state traffic (`Accept`, `Heartbeat`, `Chosen`) in FIFO order. The stream's background task maintains a `PendingMap` — a `HashMap<request_id, oneshot::Sender>` shared between the send-half (insert on enqueue) and the recv-half (remove + dispatch on response). Because each `Accept` gets its own oneshot, the leader can enqueue slot N+1's `Accept` before slot N's `Accepted` response has returned. This is the mechanism that turns the sequential `for peer in peers { send_accept().await }` loop into true parallel fanout.

**Per-follower flow control.** The stream's user-facing `cmd_tx` is a bounded `tokio::sync::mpsc` whose capacity is `peer_stream_window_frames` (default 64, tunable via `PxElectionConfig`). This mpsc acts as the per-peer in-flight cap. When the queue is full, `dispatch` fails and the proposer surfaces `PxPaxosError::Busy` — already classified `FailRetryable` in [`design-paxos-error.md`](design-paxos-error.md). If a peer falls behind beyond a configured slack (default 4 × window), the leader stops sending new `Accept`s to that peer and switches to **catch-up mode** (streaming missed slots in batched form). The peer remains a member of the group, just not part of the current quorum.
