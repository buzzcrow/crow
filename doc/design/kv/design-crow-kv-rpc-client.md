...
# CROW - Design: Client-Facing KV RPC

Depends on: [`design-crow-kv-rpc.md`](design-crow-kv-rpc.md) §3, §5, §10, §11, §12; [`design-crow-rpc.md`](../rpc/design-crow-rpc.md) §3, §4, §6
Satisfies: [`design-crow-kv.md`](design-crow-kv.md) §3, §10.1
...
- [9. CrowkvClient Transport Selection](#9-crowkvclient-transport-selection)
...
The schema lives in `lib/crow-protocol/src/fbs/kv_client.fbs`. Codegen
runs via `crow-protocol`'s `build.rs`, producing `kv_client_generated.rs`
re-exported as `crow_protocol::kv_client_fb`.
...
`KvRpcService` (in `lib/crow-kv/src/rpc/kv_rpc_service.rs`) holds
...
`KvClientRpcForwarder` lives in `crow-kv` itself (not `crow-kv-client`)
...
d. Else → build `Connection` via `from_handle`, register it in the
   group's `WatchRegistry` with a `CrowRpcPushTarget`
...
## 9. CrowkvClient Transport Selection

`CrowkvClient` holds an `Option<Arc<KvRpcTransport>>` field
...
- **crow-rpc path** (when `with_rpc_transport` is set): persistent
  ...
  dead connections — crow-rpc does not deliver a connection-close
  callback.
...
[`design-crow-rpc.md`](../rpc/design-crow-rpc.md) §6 for the wrapper
convention.
...
- [`design-crow-kv-rpc.md`](design-crow-kv-rpc.md) — consensus RPC wire
  protocol (same engine, different port + schema).
- [`design-crow-rpc.md`](../rpc/design-crow-rpc.md) — RPC engine
  overview (framing, connection pool, handler pattern, wrapper
  convention).
- [`design-crow-kv-watch-notify.md`](design-crow-kv-watch-notify.md) —
