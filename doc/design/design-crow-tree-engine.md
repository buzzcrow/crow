...
[`design-crow-tree-storage.md`](design-crow-tree-storage.md).
...
crow-tree is an ordered map `key → (slot, cell)`, where `key` is an opaque byte
...
- **L1 — B+tree.** A **single-writer / multi-reader** B+tree with
  ...
  ([`design-crow-tree-storage.md §8`](design-crow-tree-storage.md#8-mapping-table)).
...
- **Tombstone:** a delete is a cell with the tombstone flag; it occupies space
  ...
  ([`design-crow-tree-storage.md §9`](design-crow-tree-storage.md#9-garbage-collection)).
...
specified in
[`design-crow-tree-storage.md §3`](design-crow-tree-storage.md#3-on-disk-page-format-zero-copy-frame);
...
- A NoOp / empty batch still advances the contiguous frontier via
  ...
  the gap a NoOp leaves (`design-crow-tree.md §3.1`).
...
- **Option B — handle-based ownership transfer.** The caller
  allocates crow-tree-owned memory via `ct_alloc(key_len, val_len)`, which
...
Ownership rule (extends `design-crow-tree.md §4`): a buffer *yielded* to
...
- `CrowTreeEngine` regression tests (`crow_tree_engine_test.rs`): a resident
