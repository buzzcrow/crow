...
| Doc | Read when working on |
| --- | --- |
| `doc/design/protocol/design-crow-key.md` | Cross-component key encoding protocol for crow-kv: design decisions (flat per-kind struct, 3-byte header, big-endian fixed-width, null-terminated strings), unified key concept (one struct, two encoding traits: `BinaryKey` + `TextKey`), frozen binary + text key layouts, append-only evolution policy, RPC relationship, trait shape, testing strategy. Read when changing a key layout or a key/proto message. Persistence schema (which keys are stored, value types, scan patterns) is in `design-crow-kv-group0.md` §3 and `design-crow-diskdb.md` §5/§7. |

### diskdb — Disk-Block Allocator

| Doc | Read when working on |
| --- | --- |
| `doc/design/diskdb/design-crow-diskdb.md` | diskdb root design: what diskdb is (lightweight, stateless disk-block allocator on CROW KV), key design decisions (group 0 sysdata, disk-group→paxos-group bind table, records-are-source-of-truth, no CAS, `JournalScan` crow-kv extension for fast restart), architecture, group-0 sysdata schema, zone records + three recovery strategies (full scan / journal scan / compaction), allocation algorithm (round-robin across disks within the named disk-group, CAS retry bound, immediate free), state machines, disk status management (fixed 10s sync, disk-add init flow), space metrics (counters / gauges / latency hierarchy, keepalive usage summary), background scanner, configuration, implementation split (R70–R79). Read first for any diskdb design or architecture question. |
| `doc/working/design-diskdb-server.md` | diskdb server implementation design (R71 + R72), framed around two major design areas: (1) status management — group-0 sync, keepalive, disk-group/disk status changes (new disk, missing disk, Bad disk → scan impacted busy blocks for future recovery); (2) allocate/free — zone bitmap-scan allocator, rotating active-zone-set, round-robin within named disk-group, two-phase async allocate, immediate free, with crash-safety invariants for the derived zone bitmap (records are source of truth; allocate/free preserve invariants for R73 replay + compaction). Current state (R71 partial — flat module layout, `HardwareClient`/`ServiceRegistryClient` replace the planned `SysdataClient`, simplified `SyncLoop`, standalone `StatusManager`, `UsageBitmap` uses `fetch_or`/`fetch_and` not per-bit CAS), protocol enhancements (proto fixes, no new RPCs, keepalive usage piggyback), group-0 test cluster (real group 0, no mock), server wiring, config extensions, test strategy. R71 partially implemented; read when implementing R72. |
| `doc/working/design-diskdb-recovery.md` | diskdb crash recovery + snapshot compaction implementation design (R73): `JournalScan` crow-kv extension (slot-ordered WAL scan proto, server-side handler, client method, mock support), `RebuildZoneBitmap` RPC, recovery engine (strategy 2 journal scan replay — load `ZoneValue` snapshot + replay post-snapshot ops in slot order; strategy 1 full scan rebuild — scan `BusyBlockKey`s, rebuild bitmap; fallback chain), compaction engine (strategy 3 — scan free records, merge into `ZoneValue`, delete only free records, crash safety), zone struct extensions, DataGroupClient extensions, config, server wiring, module structure, test strategy. Read when implementing R73 or the `JournalScan` crow-kv extension. |
