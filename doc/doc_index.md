...
| `plan-tree.md` | ~480 | Crowtree implementation task tracker: all remaining tasks organized by layer (Overview, Memory, Async FFI, Core, Persistence, Snapshot/GC, Test, Infrastructure), with P0/P1/P2 priorities, checkboxes, dependency graph, resolved pre-impl gaps. Tasks #17/#18 (buffer pool + incremental snapshot), #19 (checkpoint→snapshot rename). Read before starting crowtree work. |
...
| `design/design-crowtree.md` | ~430 | crowtree overview: P3 decisions (D-Q1..D-Q13), architecture, redefined async `KVEngine`/`EngineView` abstraction, out-of-order apply + two-GC model (§4.1), FFI boundary, **Rust adapter `CrowtreeEngine` design (§5a)**, sub-doc map by layer. Read first for storage-engine work. |
| `design/design-crowtree-core.md` | ~600 | crowtree core (Tree & Epoch layer): slot cell, pages, **mapping table (see `design/design-crowtree-mappingtable.md`)**, delta records, write path (apply→delta→consolidate→split/merge), versioned root (`RootVersion`), tree-owned epoch GC + lock-free EBR design (§10.1), read path (incl. zero-copy value returns), concurrency invariants. MemTable = `absl::btree_map` + `buffer`. |
| `design/design-crowtree-memory.md` | ~220 | crowtree key/value memory ownership: the `buffer` abstraction (owned/borrowed, move-only, header reserve), single-allocation zero-copy write pipeline (Rust→MemTable→frame), borrowed zero-copy read views + epoch guard, Rust FFI ownership transfer (copy-at-boundary vs shared allocator), future size-classed memory pool & RDMA-pinned buffers. |
| `design/design-crowtree-async.md` | ~200 | crowtree async FFI bridge: io_uring reactor (1 thread per tree), completion-based `ct_future` protocol, fast-path (sync) / slow-path (io_uring SQE) split, Rust `Future` + `AsyncFd` on eventfd, zero-copy fast-path value, `AsyncPageStore` abstraction, macOS dev path. |
| `design/design-crowtree-persistence.md` | ~560 | crowtree persistence: `PageStore` (file/block/RDMA); **zero-copy slotted frame** format (in-mem == on-disk, §3); **buffer pool** (frame arena, pin/unpin, CLOCK eviction, 4/8 GiB cap, dirty tracking, §4); snapshot + §5A high-perf L0→L1→disk pipeline; internal-WAL decision (none; snapshot+replay); recovery; C API; **§9 snapshot export/import** (portable stream + `.ctsnap` file). LZ4 default compression. |
| `design/design-crowtree-mappingtable.md` | ~300 | crowtree mapping table (#14): Bw-Tree PID indirection, packed slot word (resident/unloaded), no PID reuse + epoch-safe segment recycling, segment-image + segment-directory + tiny A/B commit-anchor persistence, snapshot/recovery ordering, two-generation image cleanup. |
