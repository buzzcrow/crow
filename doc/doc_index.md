...
| `doc/design/chunkdb/design-crow-chunkdb-range-binding.md` | chunkdb instance sharding: non-contiguous sub-range binding schema, `BindingStrategy` trait + `ChunkdbRangeStrategy`, `RangeBindingClient` (route + transition fallback + retry), `RangeGuard` enforcement, leader-gated `BindingMonitor` in crow-kv-server, `NotMyRange` reject-and-retry, precise `free_blocks` routing. |
