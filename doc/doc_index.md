...
| `doc/working/design-diskdb-server.md` | diskdb server implementation design (R71 + R72): protocol enhancements (proto fixes for `BusyBlockValue`/`FreeBlockValue`/`ZoneValue`/`BlockState`, new RPCs), group-0 simulation (in-process mock crow-kv-compatible server for testing), status management (SysdataClient, SyncLoop, StatusManager, NodeContainer, disk-add init flow), allocate/free (zone bitmap-scan allocator, rotating active-zone-set, round-robin within named disk-group, DataGroupClient, two-phase async allocate, immediate free, gRPC handlers), server wiring, config extensions, test strategy. Read when implementing R71 or R72. |
| `doc/working/design-diskdb-sysdata.md` | diskdb group-0 sysdata + sync loop + status management implementation design (R71): protocol review (missing `InstanceValue`/`OwnerEntry`/`BindEntry` protos, `KeepaliveRequest` usage summary), group-0 simulation (in-process mock crow-kv-compatible server), SysdataClient (binary keys, protobuf values, read/write methods), SyncLoop (fixed 10s sync, change detection, epoch guard, degraded mode, R78 notify deferred), StatusManager (`HwStatus` transitions, `effective_status = max(node, group, disk)`, `Suspect` timeout, allocation gating), disk-add init flow (create zones, write baseline `ZoneValue`), NodeContainer/Node/ZoneDisk (RCU contexts, disk-id→disk-group lookup), admin gRPC handlers, server wiring, config, module structure, test strategy. Read when implementing R71. |
