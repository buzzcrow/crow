...
| M2 — Election + bulk Phase 1 | `election.rs` unit + S1 scenario | < 10 s | All pass |
| M3 — Proposer + pipeline | `proposer.rs` + `repair.rs` unit + S2 scenario | < 15 s | All pass |
| M4 — Learner + engine + lease + reads | `learner.rs` + `engine.rs` + `lease.rs` unit + S3 scenario + lease-skew/expiry/step-down tests (C10–C12) | < 15 s | All pass |
| M5 — Dedup cache | `dedup.rs` unit + C7 invariant | < 5 s | All pass |
| **G1 — Core linearizable** | All M1–M5 + 1000 random ops harness scenario | < 30 s | **All pass; gates start of P4 (P2/P3 may proceed in parallel)** |
