...
// into `crow-tree` with per-type using-declarations so existing
// `Counter*`/`Gauge*`/`LatencySummary*`/`MetricsRegistry`/`Bandwidth`
// references compile unchanged. (Not a `namespace crow::tree =
...
class Crowtree
{
  public:
    ...
    // no entry-by-entry tree rebuild. Both intended for crow-tree-to-crow-tree
    ...
    // and testing (comparable against a non-crow-tree oracle).
    ...
    // separately -- see evict_clean_inner_locked's doc comment (crow-tree.cpp)
