...
// Compile the crow-tree C++ engine (including the C ABI) into a static lib and
// link it into this crate. LZ4 is optional: set CROW_TREE_LZ4_LIB=/path/to/dir
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // crate dir = crow-tree/ffi ; engine root = crow-tree
    let manifest = PathBuf::from(std::env::var("CARGO_MANIFEST_DIR")?);
    let engine = manifest
        .parent()
        .ok_or("CARGO_MANIFEST_DIR must have a parent crow-tree dir")?
        .to_path_buf();
    let src = engine.join("src");
    let include = engine.join("include");
    // crow-common shared utils (R12): crc32c, log, compressing_sink, gzip,
    // metrics moved out of crow-tree into a sibling `crow-common/cpp` project.
    // The FFI build globs both source trees into one `cc::Build` so the moved
    // TUs compile with the same flags as the remaining crow-tree sources.
    let common = engine
        .parent()
        .expect("crow-tree must have a parent")
        .join("crow-common")
        .join("cpp");
    ...
    let have_lz4 = std::env::var("CROW_TREE_LZ4_LIB").ok();
    if have_lz4.is_some() {
        build.define("CROW_TREE_HAVE_LZ4", "1");
    }

    // ── spdlog logging (mirrors CMakeLists.txt) ──
    // The FFI build now enables spdlog so the C++ engine writes
    // `crow-tree.log` alongside the Rust server's `log/` directory.
    ...
    if have_spdlog {
        ...
        // (the remaining crow-tree sources include crow-common/log.h and use
    ...
    println!("cargo:rerun-if-env-changed=CROW_TREE_LZ4_LIB");
    println!("cargo:rerun-if-env-changed=CONDA_PREFIX");
    Ok(())
}
