...
namespace crow::tree
{
// Metrics core moved to crow-common::metrics (R12); bring the moved types
// into `crow-tree` so the test's unqualified `Counter`/`Gauge`/... references
// resolve. `Crowtree`/`Options`/`Batch`/`MemPageStore` stay in `crow-tree`.
using namespace crow::common::metrics;
