...
//!
//! See `doc/design/kv/design-crow-kv-group0.md` §4.
...
impl ServiceRegistryClient {
    /// Wrap a `CrowkvClient` for group-0 service registry access.
    #[must_use]
    pub fn new(kv: CrowkvClient) -> Self {
        Self { kv: Arc::new(kv) }
    }

    /// Wrap an already-shared `CrowkvClient` for group-0 service registry access.
    #[must_use]
...
impl ServiceRegistryClient {
    ...
    pub async fn register_kv_server(
        ...
        let extra = ServiceExtra {
            diskdb: None,
            kv_server: Some(crow_protocol::common::KvServerExtra {
