...
//! `CrowClusterDeployer` — reusable cluster lifecycle manager.
//!
...
//!
//! The deployer talks to a `crow-web` instance via `ConsoleClient`.
//! It does not spawn `crow-web` itself — the caller is responsible
...
/// cycle. The caller starts the `crow-web` instance (embedded or
/// external) and passes its base URL to [`CrowClusterDeployer::new`].
pub struct CrowClusterDeployer {
...
impl CrowClusterDeployer {
    /// Create a new deployer pointing at a running `crow-web` instance.
    ///
    /// # Errors
...
impl Drop for CrowClusterDeployer {
    fn drop(&mut self) {
        if !self.deployed_node_ids.is_empty() {
            tracing::warn!(
                "CrowClusterDeployer dropped without stop() — {} nodes still deployed",
