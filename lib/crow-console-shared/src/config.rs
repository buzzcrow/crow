...
//! Console configuration: persisted registry of `crow-kv-server` instances.
//!
...
impl TomlFileEngine {
    ...
    pub fn default_path() -> Option<PathBuf> {
        Some(PathBuf::from("runtime-data/crow-kv.db.toml"))
    }
...
pub struct ServerEntry {
    ...
    pub id: String,
    /// `crow-kv-server` management base URL, e.g. `http://127.0.0.1:9910`.
    pub url: String,
...
mod tests {
    ...
    fn default_path_points_to_runtime_data() {
        assert_eq!(
            ...
            std::path::PathBuf::from("runtime-data/crow-kv.db.toml")
        );
        assert_eq!(
