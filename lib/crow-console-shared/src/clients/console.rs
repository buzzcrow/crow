...
//! HTTP client for `crow-web`'s two-tree API contract.
//!
//! `ServerClient` (sibling) targets a single `crow-kv-server`'s
//! management API and is used by `crow-web` itself. `ConsoleClient`
//! lives one layer higher: it talks to `crow-web` over the public
...
//! `crow-web` server-side, so there is no `?server=` parameter).
...
/// Thin wrapper around `reqwest::Client` bound to one `crow-web`
...
// ── Request bodies (match the handlers in crow-web/src/{mgmt,kv}.rs) ──
...
impl ConsoleClient {
    ...
    /// Restart the `crow-kv-server` running on `node_id`. Stops the
    ...
    // ── HTTP plumbing ──────────────────────────────────────────────
    //
    // Every helper attaches the current `x-crow-kv-corr-id` header (see
