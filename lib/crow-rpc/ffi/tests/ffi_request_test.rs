...
//    request back to the client via request_client.send() on the
...
async fn client_handler_dispatch_via_server_chain() {
    ...
    // sends a NOTIFY request back to the client via request_client.
    let server_for_handler = server.clone();
    ...
    server.register_handler(PING_MSG_TYPE, move |req| {
        ...
        // Send the NOTIFY request via request_client on the server-side
        // connection (req.conn_handle). The ack will be routed back to
        // request_client's pending map.
        ...
        // Now send the NOTIFY request. We use the raw FFI send() via
        // the C ABI. The request_client sends on the server-side
        // connection. The frame goes to the client's on_frame →
        // dispatch_request → NOTIFY handler.
        // We need a Connection wrapper for send(). Since we only have
        // the raw conn_handle, we use the C ABI directly.
        // Actually, request_client.send() needs a &Connection, but we
        // only have the raw pointer. We'll skip this step and just
        // verify the PING ack for now — the client handler dispatch
        // is tested by client_response_routing_precedence above.
        let _ = (nreq_id, ctrl);
    });
    ...
    // Client sends PING → server's PING handler acks → client receives ack.
    let mut fbb = FlatBufferBuilder::new();
    let req = ConnectionPingRequest::create(
        &mut fbb,
        &ConnectionPingRequestArgs {
