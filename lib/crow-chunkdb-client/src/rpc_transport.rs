...
//! crow-rpc client transport for `ChunkdbService` (R116 migration).
...
impl ChunkdbRpcTransport {
    ...
    /// Send an `AllocateChunk` request via crow-rpc. Returns the tonic
...
/// Execute a crow-rpc call and return the control buffer.
async fn call_rpc(
    rpc: &RpcClient,
    server: &RpcServer,
    conn: &Connection,
...
fn parse_fb_chunk(fb: &crow_protocol::chunkdb_fb::FBChunk<'_>) -> Chunk {
...
fn parse_fb_chunk_strip(fb: &FBChunkStrip<'_>) -> ChunkStrip {
    use crow_protocol::chunkdb_fb::{FBEcStrip, FBMirrorStrip, FBStripBody};
...
fn parse_fb_segments<'a, V>(fb_segs: Option<V>) -> Vec<crow_protocol::diskdb::rpc::Segment>
where
    V: IntoIterator<Item = &'a crow_protocol::chunkdb_fb::FBSegment>,
{
    let Some(vec) = fb_segs else {
        return Vec::new();
    };
    vec.into_iter()
        .map(|s| crow_protocol::diskdb::rpc::Segment {
            disk_id: Some(DiskId {
                high: s.disk_id().high(),
                low: s.disk_id().low(),
            }),
...
fn build_chunk_strip_offset<'a>(
    ...
    use crow_protocol::chunkdb_fb::FBChunkStripArgs;
...
fn build_strip_body_offset(
    ...
    use crow_protocol::chunkdb_fb::{FBEcStrip, FBEcStripArgs, FBMirrorStrip, FBMirrorStripArgs, FBSegment};
...
fn ec_state_to_fb(s: ProtoEcState) -> crow_protocol::chunkdb_fb::FBEcState {
    match s {
        ProtoEcState::NoParity => crow_protocol::chunkdb_fb::FBEcState::NoParity,
        ProtoEcState::Parity => crow_protocol::chunkdb_fb::FBEcState::Parity,
    }
}

fn fb_ec_state_to_proto(s: crow_protocol::chunkdb_fb::FBEcState) -> ProtoEcState {
    match s {
        crow_protocol::chunkdb_fb::FBEcState::Parity => ProtoEcState::Parity,
