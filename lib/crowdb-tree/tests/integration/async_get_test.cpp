...
// the underlying Crowdbtree::*_async methods -- these are the four cases
...
// for. On a build with liburing (CROWDB_HAVE_LIBURING), ct_open wires a
...
TEST(AsyncGet, MissAfterEvictionCompletesViaReactor)
{
    ...
    // materializes an owned copy (materialize_owned(), crowdb-tree.cpp --
...
// + submit_fsync through the Reactor's io_uring on Linux (CROWDB_HAVE_LIBURING).
