...
use crow_test_harness::chunkdb::{self as cdb_harness, ChunkdbProcess};
use crow_test_harness::cluster::KvCluster;
use crow_test_harness::diskdb::{self as ddb_harness, DiskdbProcess};
use crow_test_harness::diskio::{self as dio_harness, DiskioProcess, DiskioStartOpts};
use crow_test_harness::hardware::{make_disk_id, seed_hardware, DG_ID, NODE_ID, RACK_ID, UNIT_SIZE_BYTES};
...
fn check_all_binaries() -> bool {
    if std::env::var("CROW_KV_SERVER_BIN").is_err()
        && crow_test_harness::cluster::crow_kv_server_bin().is_none()
    {
        eprintln!("skipping: crow-kv-server binary not found");
        return false;
    }
    if ddb_harness::crow_diskdb_bin().is_none() {
        eprintln!("skipping: crow-diskdb binary not found");
        return false;
    }
    if dio_harness::crow_diskio_bin().is_none() {
        eprintln!("skipping: crow-diskio binary not found");
        return false;
    }
    if cdb_harness::crow_chunkdb_bin().is_none() {
        eprintln!("skipping: crow-chunkdb binary not found");
        return false;
    }
    true
}
...
async fn start_e2e_stack() -> E2eStack {
    ...
    eprintln!("crow-diskdb ready");

    // 4. Start diskio (block I/O, mem backend).
    eprintln!("=== starting crow-diskio (mem) ===");
