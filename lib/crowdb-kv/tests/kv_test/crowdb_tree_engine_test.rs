...
//! `CrowdbTreeEngine` behavior: same shared `KVEngine` conformance suite as
//! `mem_kv_test.rs` (see `conformance.rs`), proving `CrowdbTreeEngine` and
...
/// `Crowdbtree::io_failed` from Rust (the C++ side's own fault-injection
/// coverage lives in `crowdb-tree/tests/integration/crash_recovery_test.cpp`),
/// so this only guards the clean-state default -- that `is_healthy` is
/// wired to a real call, not hardcoded `true` on `CrowdbTreeEngine` the way
...
/// `clear_drops_all_state` for the crowdb-tree-backed engine now that
/// `CrowdbTreeEngine::clear` is wired to `Crowdbtree::clear` instead of
/// panicking.
#[test]
...
async fn clear_then_persist_survives_reopen() {
    ...
    // `Crowdbtree::open`'s doc comment in persist.cpp), so this first read
...
/// op stream to `InMemKV` and `CrowdbTreeEngine`, `compare` must be empty.
#[test]
