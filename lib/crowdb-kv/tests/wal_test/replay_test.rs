...
/// caller-supplied engine (here an in-memory `CrowdbTreeEngine`, standing in
/// for the durable file-backed one `crowdb-kv-server` uses), not just the
...
/// real restart sees from a durable `CrowdbTreeEngine`) skips re-`learn()`ing
...
async fn restore_from_replay_with_engine_falls_back_when_resume_slot_has_no_accepted_entry() {
    ...
    // watermark (crowdb-tree folds `received_slots_` forward from its own
