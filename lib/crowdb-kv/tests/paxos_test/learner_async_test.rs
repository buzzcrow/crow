...
//! `CrowdbTreeEngine::get` constructs one in isolation
//! (`kv/crowdb_tree_engine_test.rs`'s `get_constructs_pending_for_genuine_demand_load_miss`
...
/// `CrowdbTreeEngine`, resolves correctly whether the underlying
...
async fn engine_get_resolves_correctly_across_both_ready_and_pending() {
    ...
    // Grab a shared handle to the underlying `Crowdbtree` *before* boxing
    // `engine` into `PxLearner` -- `Arc<Crowdbtree>`, so flush/snapshot/evict
    ...
    // guard in `kv/crowdb_tree_engine_test.rs` for the isolated version of
    // this same property).
