...
//! the call sites (`crowdb-kv-server` CLI, testkit harness) before the
...
pub struct ServerConfig {
    ...
    /// static: number of crowdb-rpc connections per peer endpoint for
...
/// Unified configuration for a `CrowDB` cluster node.
///
...
/// loading.
///
/// Usage: `crowdb_common::config::load_from_file::<CrowDBConfig>(path)`
/// or `CrowDBConfig::default()`, then CLI args override individual
/// fields, then pass `&CrowDBConfig` to `create_group_with_wal`.
///
/// All top-level fields are static (bind at startup / group creation).
/// Dynamic fields are inside `WalConfig` (GC tunables).
#[derive(Clone, Debug, serde::Deserialize, serde::Serialize)]
#[allow(clippy::struct_excessive_bools)]
pub struct CrowDBConfig {
    #[serde(default)]
...
impl CrowDBConfig {
    ...
    /// `crowdb_common::config::load_from_file` (fills skip defaults +
    ...
    pub fn load_from_file(path: &std::path::Path) -> Result<Self, String> {
        crowdb_common::config::load_from_file::<Self>(path)
    }
...
mod tests {
    ...
    #[test]
    fn crowdb_kv_config_default_round_trip() {
        let config = CrowDBConfig::default();
        let toml_str = toml::to_string(&config).unwrap();
        let restored: CrowDBConfig = toml::from_str(&toml_str).unwrap();
    ...
    #[test]
    fn crowdb_kv_config_partial_toml_uses_defaults() {
        let toml_str = "wal_early_ack = true";
        let config: CrowDBConfig = toml::from_str(toml_str).unwrap();
    ...
    fn tracked_kv_server_config_file_loads() {
        ...
        let config_path = manifest_dir
            ...
            .join("..")
            .join("app")
            .join("crowdb-kv-server")
            .join("conf")
            .join("crowdb_kv_server_config.toml");
