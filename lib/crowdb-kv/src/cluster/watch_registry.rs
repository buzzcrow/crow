...
/// crowdb-rpc push target for `WatchNotify` (R117). The server pushes
...
impl CrowdbRpcPushTarget {
    #[must_use]
    pub fn new(
        conn: crowdb_rpc_ffi::Connection,
        rpc: Arc<crowdb_rpc_ffi::RpcClient>,
        server: Arc<crowdb_rpc_ffi::RpcServer>,
    ) -> Self {
        Self {
            conn,
            rpc,
...
/// One watcher: an outbound push target. crowdb-rpc watchers use a
/// `CrowdbRpcPushTarget` (persistent connection + fire-and-forget send).
pub enum PushTarget {
    CrowdbRpc(Arc<CrowdbRpcPushTarget>),
}
...
impl WatchRegistry {
    ...
    /// Build + send a `FBWatchNotify` frame to a crowdb-rpc watcher.
    ...
    fn push_crow_rpc_notify(
        ...
        let key_offsets: Vec<_> = keys
            ...
            .map(|k| {
                ...
                crowdb_protocol::kv_client_fb::FBBytes::create(
                    &mut builder,
                    &crowdb_protocol::kv_client_fb::FBBytesArgs { data: Some(data) },
                )
            })
            .collect();
        ...
        let args = crowdb_protocol::kv_client_fb::FBWatchNotifyArgs {
            id: push_id,
            rpc_create_nano: 0,
            group_id,
        ...
        let fb = crowdb_protocol::kv_client_fb::FBWatchNotify::create(&mut builder, &args);
        ...
        if let Err(e) = result {
            if matches!(
                e,
                crowdb_rpc_ffi::RpcError::ConnectionClosed | crowdb_rpc_ffi::RpcError::ConnectionError
            ) {
                self.closed_watchers.fetch_add(1, Ordering::Relaxed);
                tracing::warn!(
                    "watch notify: crowdb-rpc push failed ({e}) -- \
                     lazy watcher removal on next emit pass"
                );
            } else {
                self.dropped_notifies.fetch_add(1, Ordering::Relaxed);
                tracing::error!(
                    "watch notify: crowdb-rpc push error ({e}) -- \
