...
//! `PxLocalReplica` — local acceptor/learner for a single `CrowDB` group member.
//!
...
impl PxLocalReplica {
    ...
    pub fn status(&self) -> ReplicaStatus {
        ...
        let crowtree_stats = self
            ...
            .engine()
            .as_any()
            .downcast_ref::<crate::kv::CrowdbTreeEngine>()
            .map(|e| crate::cluster::status::crowdb_tree_stats_to_view(e.stats()));
