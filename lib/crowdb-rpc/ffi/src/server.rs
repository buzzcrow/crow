...
impl RpcServer {
    ...
    pub fn with_engines(pool: Option<&crate::BufferPool>, io_engines: u32, io_workers: u32) -> Self {
        ...
        let effective_workers = if cfg!(target_os = "macos") && io_workers > 1 {
            MACOS_CLAMP_WARNED.call_once(|| {
                eprintln!(
                    "warn: crowdb-rpc: macOS kqueue clamps I/O workers to 1 (requested {io_workers}); \
    ...
    pub fn listen(&self, addr: &str, port: i32) -> Result<(), RpcError> {
        ...
        let status = unsafe { sys::crowdb_rpc_server_listen(self.handle, c_addr.as_ptr(), port) };
    ...
    pub fn start(&self) {
        unsafe { sys::crowdb_rpc_server_start(self.handle) };
    }
    ...
    pub fn port(&self) -> i32 {
        unsafe { sys::crowdb_rpc_server_port(self.handle) }
    }
    ...
    pub fn set_send_queue_capacity(&self, capacity: u32) {
        unsafe { sys::crowdb_rpc_server_set_send_queue_capacity(self.handle, capacity) };
    }

    /// TCP_NODELAY for new connections. Default true (Nagle disabled).
    pub fn set_tcp_nodelay(&self, enabled: bool) {
        unsafe { sys::crowdb_rpc_server_set_tcp_nodelay(self.handle, if enabled { 1 } else { 0 }) };
    }
    ...
    pub fn set_quickack(&self, enabled: bool) {
        unsafe { sys::crowdb_rpc_server_set_quickack(self.handle, if enabled { 1 } else { 0 }) };
    }
    ...
    pub fn set_event_write(&self, enabled: bool) {
        unsafe { sys::crowdb_rpc_server_set_event_write(self.handle, if enabled { 1 } else { 0 }) };
    }
    ...
    pub fn register_conn_count_gauge(&self, name: &str) {
        ...
        unsafe { sys::crowdb_rpc_server_register_conn_count_gauge(self.handle, c_name.as_ptr()) };
    }

    /// Sample transport-level stats: submit_to_writev latency histogram.
    pub fn transport_stats(&self) -> sys::CrowdbRpcTransportStats {
        let mut stats = sys::CrowdbRpcTransportStats::default();
        unsafe { sys::crowdb_rpc_server_transport_stats(self.handle, &mut stats) };
        stats
    }
    ...
    pub fn connect(&self, addr: &str, port: i32) -> Result<Connection, RpcError> {
        let c_addr = CString::new(addr).map_err(|_| RpcError::InvalidArg)?;
        let handle = unsafe { sys::crowdb_rpc_connect(self.handle, c_addr.as_ptr(), port) };
        if handle.is_null() {
    ...
    pub fn register_echo_handler(&self, msg_type: u16) {
        unsafe { sys::crowdb_rpc_server_register_echo_handler(self.handle, msg_type) };
    }
    ...
    pub unsafe fn submit_response(
        ...
        let status = unsafe {
            sys::crowdb_rpc_server_submit_response(
                self.handle,
                conn_handle,
                control.as_ptr(),
                control.len() as u32,
    ...
    pub unsafe fn submit_response_buffer(
        ...
        let status = unsafe {
            sys::crowdb_rpc_server_submit_response_buffer(
                self.handle,
                conn_handle,
                ctrl_handle,
                data_handle,
    ...
    pub fn set_request_client(&self, client: &crate::RpcClient) {
        unsafe { sys::crowdb_rpc_server_set_request_client(self.handle, client.handle()) };
    }

    pub fn handle(&self) -> sys::crowdb_rpc_server_t {
        self.handle
    }
}
...
impl Connection {
    ...
    /// `RpcClient::send`/`call` — those expect a `crowdb_rpc_conn_s*`
...
impl RpcError {
    pub fn from_status(status: i32) -> Self {
        match status {
            sys::CROWDB_RPC_OK => RpcError::Ok,
            sys::CROWDB_RPC_ERR_CONN_CLOSED => RpcError::ConnectionClosed,
            sys::CROWDB_RPC_ERR_TIMEOUT => RpcError::Timeout,
            sys::CROWDB_RPC_ERR_SEND_QUEUE => RpcError::SendQueueFull,
            sys::CROWDB_RPC_ERR_CONN_ERROR => RpcError::ConnectionError,
            sys::CROWDB_RPC_ERR_REGISTRATION => RpcError::RegistrationFailed,
            sys::CROWDB_RPC_ERR_ALL_DOWN => RpcError::AllDown,
            sys::CROWDB_RPC_ERR_INVALID_ARG => RpcError::InvalidArg,
...
// is safe to send because crowdb_rpc_frame_release is thread-safe (plain
