...
use crow_kv::common::logging::RotatingLogWriter;
use crow_kv::metrics::{MetricsRegistry, MetricsRunner};
use std::sync::Arc;
use std::sync::Mutex;
...
async fn registry_start_stop_lifecycle() {
    ...
    let mut runner = MetricsRunner::new(
        crow_kv::common::logging::open_metrics_log(tmp.path(), "test", 30, 5).unwrap(),
        1, // 1 second interval
    );
    runner.start();
...
async fn wal_fsync_and_write_bw_counts_match() {
    use crow_kv::paxos::roles::{PxBallot, SlotIndex};
    use crow_kv::wal::{IoBackend, RecordType, WALRecord, WalConfig, WalEngine};

    let dir = std::env::temp_dir().join(format!(
        "crow_kv_wal_metrics_test_{}_{}",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
