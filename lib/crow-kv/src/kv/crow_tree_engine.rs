...
pub use crow_tree_ffi::Options as CrowTreeOptions;
pub use crow_tree_ffi::PageStoreBackend as CrowTreeBackend;
pub use crow_tree_ffi::Stats as CrowTreeStats;

/// `KVEngine` backed by [`crow_tree_ffi::Crowtree`], via [`AsyncCrowtree`].
///
...
/// wiring landed. `apply` stays [`KVFuture::ready`]-only: crow-tree has no
/// `ct_apply_*_async` C API yet (only `ct_get_async`/`ct_scan_async`/
/// `ct_flush_async`/`ct_snapshot_async` exist), so it can't
/// genuinely wait on the reactor today; a synchronous crow-tree call in its
...
impl CrowTreeEngine {
    ...
    /// in-memory). See [`crow_tree_ffi::Crowtree::open`].
    ///
    /// # Errors
...
impl KVEngine for CrowTreeEngine {
    ...
    fn apply(&self, slot: u64, batch: &Batch) -> KVFuture<Result<(), String>> {
        ...
        // R30 zero-copy apply: the value Bytes are borrowed by crow-tree via
        ...
        // borrowed Batch is zero-copy. crow-tree enforces per-key
    ...
    fn clear(&self) {
        // `Crowtree::clear` (crow-tree/ffi) wraps the new `ct_clear` C API
    ...
    fn resume_from_slot(&self) -> u64 {
        ...
        // in crow-tree.cpp) -- a durable, gap-free watermark, not just "the max
    ...
    fn snapshot_export(&self) -> Result<(u64, Vec<u8>), String> {
        ...
        let at_slot = crow_tree_snapshot_at_slot(&stream)?;
        Ok((at_slot, stream))
    }
...
/// Parse the `at_slot` field out of a crow-tree portable-format snapshot
/// export stream's header, without waiting on a second FFI round-trip
/// ([`crow_tree_ffi::Crowtree::last_applied_slot`]) that could race a
/// concurrent `apply`/`flush` between the two calls.
///
/// Portable header layout (`crow-tree/src/snapshot_io.cpp`'s `kSnapHeader`,
/// little-endian): `[magic:u32][version:u32][format:u8][at_slot:u64]
/// [entry_count:u64]`. `ct_snapshot_export_begin` always uses
/// `snapshot_format::kPortable` (`crow-tree/src/c_api.cpp`) -- crow-tree's C
/// API has no format parameter, so this layout is the only one
/// [`crow_tree_ffi::Crowtree::snapshot_export`] can ever produce.
fn crow_tree_snapshot_at_slot(stream: &[u8]) -> Result<u64, String> {
    stream
        .get(9..17)
        .map(|b| u64::from_le_bytes(b.try_into().expect("slice len checked by get(9..17)")))
        .ok_or_else(|| "crow-tree snapshot export: stream too short for header".to_string())
}
