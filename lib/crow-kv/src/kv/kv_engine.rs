...
pub trait KVEngine: Send + Sync {
    ...
    /// returned, enabling cursor-based pagination. Entry keys and values
    /// are zero-copy `Bytes`: for `CrowTreeEngine` they slice into the
    /// C++ packed result buffer; for `InMemKV` they are `Bytes::from` of
    /// the owned `Vec<u8>` (one copy, same as its get path). Errors (e.g.
