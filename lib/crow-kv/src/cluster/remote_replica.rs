...
//! `PxRemoteReplica` — crow-rpc adapter for a peer replica in a Paxos group.
//!
...
//! bridges over crow-rpc, fire-and-forget `ChosenNotification` /
...
pub struct PxRemoteReplica {
    ...
    /// Per-RPC deadline for the crow-rpc unary calls (`Prepare`, `Accept`,
...
impl PxRemoteReplica {
    ...
    /// in `group_id`. Sent via crow-rpc with no reply awaited; failures
    ...
    /// end_slot]`. Sent via crow-rpc before the full-accept catch-up loop
    ...
    /// Get the crow-rpc transport, cloning the `Arc` for use in an async
    ...
    /// (if it was ever initialized). The crow-rpc transport is shared and
