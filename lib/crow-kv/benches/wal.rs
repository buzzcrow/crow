...
use crow_kv::wal::{BlockDevice, IoBackend, MemBlockDevice, WalConfig};
...
fn main() {
    ...
    let force_run = std::env::var("CROW_KV_RUN_WAL_BENCH")
        .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
        .unwrap_or(false);
    if cfg!(test) && !force_run {
        eprintln!("Skipping wal bench in cargo test mode; set CROW_KV_RUN_WAL_BENCH=1 to run real results.");
        return;
    }
