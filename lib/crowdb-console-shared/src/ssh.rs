...
//! SSH transport for the `CrowDB Storage` Console.
//!
...
impl Session {
    ...
    pub(crate) async fn connect(node: &NodeEntry, creds: &SshCreds) -> Result<Self> {
        ...
        // be determined (no $HOME and no $CROWDB_KV_KNOWN_HOSTS) we fall
...
/// Run a no-op command (`echo crowdb-kv-ping`) and assert the round trip.
///
/// # Errors
...
pub async fn probe(node: &NodeEntry) -> Result<()> {
    ...
    let out = session.exec("echo crowdb-kv-ping").await?;
    session.close().await;
    ...
    if !out.stdout_str().contains("crowdb-kv-ping") {
        return Err(Error::UpstreamRpc {
            node_id: node.host.clone(),
...
/// the path (`$CROWDB_KV_KNOWN_HOSTS` or `~/.crowdb-kv/known_hosts`) and
...
fn known_hosts_store_for_session() -> Result<Arc<KnownHostsStore>> {
    ...
    let path = KnownHostsStore::default_path().unwrap_or_else(|| {
        ...
        std::env::temp_dir().join(format!("crowdb-kv-known_hosts-{}", std::process::id()))
    });
...
/// `crowdb_console_shared::lifecycle::remote_start_command`, capture the
