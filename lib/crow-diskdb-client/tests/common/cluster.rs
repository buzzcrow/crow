...
//! Shared cluster harness for diskdb-client integration tests.
//!
//! Starts a real single-node `crow-kv-server` cluster with store 0,
//! group 0 (system) and group 1 (data), wires topology, waits for
//! leader election, and provides helpers to build clients seeded with
//! the group-0 leader endpoint. No diskdb dependency — the diskdb
//! binary is started separately by the test.
...
/// Get the gRPC endpoint of the leader node for a group.
...
/// A running single-node kv-server cluster with group 0 (system) and
/// group 1 (data). A single replica elects itself leader immediately
/// — no need for 3 nodes in a diskdb-client smoke test.
