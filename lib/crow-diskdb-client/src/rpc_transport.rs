...
//! crow-rpc transport for `DiskdbClient` (R115 migration).
//!
...
impl DiskdbRpcTransport {
    ...
    /// Allocate blocks via crow-rpc.
    pub async fn allocate_blocks(
        &self,
        rpc_endpoint: &str,
        req: &crow_protocol::diskdb::rpc::AllocateBlocksRequest,
    ...
    /// Free blocks via crow-rpc.
    pub async fn free_blocks(
        &self,
        rpc_endpoint: &str,
        req: &crow_protocol::diskdb::rpc::FreeBlocksRequest,
    ) -> Result<FreeResponse> {
        let req_id = self.next_id();
        let conn = self.conn_for(rpc_endpoint)?;
    ...
    /// Commit blocks via crow-rpc.
    pub async fn commit_blocks(
        &self,
        rpc_endpoint: &str,
        req: &crow_protocol::diskdb::rpc::CommitBlocksRequest,
    ) -> Result<crow_protocol::diskdb::rpc::CommitBlocksResponse> {
        let req_id = self.next_id();
        let conn = self.conn_for(rpc_endpoint)?;
    ...
    pub async fn query_capacity_stats(
        &self,
        rpc_endpoint: &str,
        req: &crow_protocol::diskdb::rpc::QueryCapacityStatsRequest,
    ...
    /// Get disk-group info via crow-rpc.
    ...
    /// Rebuild zone bitmap via crow-rpc.
    ...
    /// Recalc disk usage via crow-rpc.
    pub async fn recalc_disk_usage(
        &self,
        rpc_endpoint: &str,
        req: &crow_protocol::diskdb::rpc::RecalcDiskUsageRequest,
    ...
    /// Compact zone via crow-rpc.
    pub async fn compact_zone(
        &self,
        rpc_endpoint: &str,
        req: &crow_protocol::diskdb::rpc::CompactZoneRequest,
...
fn build_allocate_request(req_id: u64, req: &crow_protocol::diskdb::rpc::AllocateBlocksRequest) -> Buffer {
...
fn build_free_request(req_id: u64, req: &crow_protocol::diskdb::rpc::FreeBlocksRequest) -> Buffer {
    let mut fbb = FlatBufferBuilder::new();
...
fn build_commit_request(req_id: u64, req: &crow_protocol::diskdb::rpc::CommitBlocksRequest) -> Buffer {
    let mut fbb = FlatBufferBuilder::new();
    let segs = build_segments(&mut fbb, &req.segments);
...
fn build_recalc_request(req_id: u64, req: &crow_protocol::diskdb::rpc::RecalcDiskUsageRequest) -> Buffer {
    let mut fbb = FlatBufferBuilder::new();
...
fn build_compact_zone_request(req_id: u64, req: &crow_protocol::diskdb::rpc::CompactZoneRequest) -> Buffer {
    let mut fbb = FlatBufferBuilder::new();
...
fn check_ret_code(code: FBDiskdbRetCode, msg: Option<&str>) -> Result<()> {
    ...
    Err(match code {
        ...
        FBDiskdbRetCode::NotOwner => DiskdbClientError::Rpc(format!("not owner: {msg}")),
...
fn parse_allocate_response(resp: &crow_rpc_ffi::Response) -> Result<AllocateResponse> {
    let ctrl = resp
        .control
        .as_ref()
...
fn parse_free_response(resp: &crow_rpc_ffi::Response) -> Result<FreeResponse> {
    let ctrl = resp
        .control
        .as_ref()
        .ok_or_else(|| DiskdbClientError::Rpc("missing control buffer".into()))?;
...
fn parse_query_capacity_response(resp: &crow_rpc_ffi::Response) -> Result<QueryCapacityStatsResponse> {
    let ctrl = resp
        .control
        .as_ref()
        .ok_or_else(|| DiskdbClientError::Rpc("missing control buffer".into()))?;
...
fn parse_get_disk_group_info_response(resp: &crow_rpc_ffi::Response) -> Result<GetDiskGroupInfoResponse> {
    let ctrl = resp
        .control
        .as_ref()
        .ok_or_else(|| DiskdbClientError::Rpc("missing control buffer".into()))?;
...
fn parse_get_disk_info_response(resp: &crow_rpc_ffi::Response) -> Result<GetDiskInfoResponse> {
    let ctrl = resp
        .control
        .as_ref()
        .ok_or_else(|| DiskdbClientError::Rpc("missing control buffer".into()))?;
...
fn parse_rebuild_zone_bitmap_response(resp: &crow_rpc_ffi::Response) -> Result<RebuildZoneBitmapResponse> {
    let ctrl = resp
        .control
        .as_ref()
        .ok_or_else(|| DiskdbClientError::Rpc("missing control buffer".into()))?;
...
fn parse_recalc_response(resp: &crow_rpc_ffi::Response) -> Result<RecalcDiskUsageResponse> {
    let ctrl = resp
        .control
        .as_ref()
        .ok_or_else(|| DiskdbClientError::Rpc("missing control buffer".into()))?;
    ...
    let results: Vec<crow_protocol::diskdb::rpc::DiskGroupRecalcResult> = r
        ...
        .map(|v| {
            v.iter()
                .map(|dg_r| {
                    let zone_results: Vec<crow_protocol::diskdb::rpc::ZoneRecalcResult> = dg_r
                        .zones()
                        .map(|zv| {
                            zv.iter()
                                .map(|zr| {
                                    ...
                                    crow_protocol::diskdb::rpc::ZoneRecalcResult {
                                        disk_id: Some(DiskId {
                                            high: did.high(),
                                            low: did.low(),
                                        }),
                        ...
                        .unwrap_or_default();
                    crow_protocol::diskdb::rpc::DiskGroupRecalcResult {
...
fn parse_compact_zone_response(resp: &crow_rpc_ffi::Response) -> Result<CompactZoneResponse> {
    let ctrl = resp
        .control
        .as_ref()
        .ok_or_else(|| DiskdbClientError::Rpc("missing control buffer".into()))?;
    ...
    let zones: Vec<crow_protocol::diskdb::rpc::ZoneCompactionResult> = r
        ...
        .map(|v| {
            v.iter()
                .map(|z| crow_protocol::diskdb::rpc::ZoneCompactionResult {
...
fn parse_trigger_scan_response(resp: &crow_rpc_ffi::Response) -> Result<TriggerScanResponse> {
    let ctrl = resp
        .control
        .as_ref()
        .ok_or_else(|| DiskdbClientError::Rpc("missing control buffer".into()))?;
...
fn parse_get_scan_status_response(resp: &crow_rpc_ffi::Response) -> Result<GetScanStatusResponse> {
    let ctrl = resp
        .control
        .as_ref()
        .ok_or_else(|| DiskdbClientError::Rpc("missing control buffer".into()))?;
...
fn parse_disk_group_info(fb: FBDiskGroupInfo) -> crow_protocol::diskdb::rpc::DiskGroupInfo {
    ...
    let disks: Vec<DiskInfo> = fb
        ...
        .unwrap_or_default();
    crow_protocol::diskdb::rpc::DiskGroupInfo {
        rack_id: fb.rack_id(),
        node_id: fb.node_id(),
...
fn parse_scan_summary(fb: FBScanSummary) -> crow_protocol::diskdb::rpc::ScanSummary {
    crow_protocol::diskdb::rpc::ScanSummary {
...
fn fb_hw_status(v: FBHwStatus) -> i32 {
    match v {
        FBHwStatus::Up => crow_protocol::common::HwStatus::Up as i32,
        FBHwStatus::Maintenance => crow_protocol::common::HwStatus::Maintenance as i32,
        FBHwStatus::Suspect => crow_protocol::common::HwStatus::Suspect as i32,
        FBHwStatus::Missing => crow_protocol::common::HwStatus::Missing as i32,
        FBHwStatus::Bad => crow_protocol::common::HwStatus::Bad as i32,
        FBHwStatus::Offline => crow_protocol::common::HwStatus::Offline as i32,
        _ => crow_protocol::common::HwStatus::Init as i32,
    }
}
