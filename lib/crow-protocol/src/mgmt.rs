...
// and therefore live here per `design-crow-kv-group0.md` §2.4.
...
pub struct KvStoreStatus {
    ...
    pub key_count: u64,
    /// `true` for `InMemKV` always; for a `CrowTreeEngine`, `false`
    /// once a durable I/O fault has latched.
    #[serde(default = "default_true")]
    pub engine_healthy: bool,
    /// `CrowTreeEngine` stats, or `None` for `InMemKV`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Wire-serializable mirror of `CrowTreeStats` (that type lives in
/// `crow_tree_ffi` and isn't `Serialize`), for `/topology`/`/health`.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, Serialize, Deserialize)]
#[cfg_attr(feature = "schema", derive(utoipa::ToSchema))]
