...
async fn multi_group_routes_to_correct_group() {
    ...
    // Write to group 1
    let r1 = store
        .kv_put(1, b"g1-key".to_vec(), b"g1-val".to_vec(), 1, 1, 100, 1000)
        .await;
    assert!(r1.ok, "group 1 put should succeed");

    // Write to group 2
    let r2 = store
        .kv_put(2, b"g2-key".to_vec(), b"g2-val".to_vec(), 2, 1, 200, 2000)
        .await;
...
async fn missing_group_returns_error() {
    ...
    let resp = store
        .kv_put(99, b"k".to_vec(), b"v".to_vec(), 1, 1, 100, 1000)
        .await;
    assert!(!resp.ok);
    assert!(
    ...
    let resp = store
        .kv_batch_write(99, vec![], 1, 3, 102, 1002)
        .await;
    assert!(!resp.ok);
    assert!(resp.error.contains("no kv group"));
...
async fn add_and_remove_group_dynamic() {
    ...
    let r = store
        .kv_put(1, b"k".to_vec(), b"v".to_vec(), 1, 1, 100, 1000)
        .await;
    assert!(r.ok);
    ...
    let r = store
        .kv_put(1, b"k".to_vec(), b"v".to_vec(), 1, 2, 101, 1001)
        .await;
    assert!(!r.ok);
