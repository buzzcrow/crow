...
async fn forwarded_request_does_not_re_forward() {
    ...
    assert!(
        !resp.ok,
        "forwarded request must serve locally; expected not_found, got {resp:?}"
    );
    assert!(
        resp.not_found,
        "expected not_found when loop-guard is set and local store was cleared"
    );
