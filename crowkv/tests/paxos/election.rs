...
fn noop_apply_path() {
    ...
    assert_eq!(learner.contiguous_chosen(), 1, "NoOp should advance contiguous_chosen");
    assert_eq!(learner.contiguous_applied(), 1, "NoOp should advance contiguous_applied alongside chosen");
}
...
async fn prevote_does_not_bump_term() {
    ...
    .await;
    assert_eq!(replica.current_term_snapshot(), term_before, "PreVote must never bump current_term");
}
...
async fn term_fencing_in_acceptor_adopts_higher_term() {
    ...
    assert_eq!(replica.current_term_snapshot(), 9, "current_term must be adopted from the higher-term Accept");
}
