...
async fn dedup_lookup_returns_none_for_client_id_zero() {
    ...
    learner.learn(entry, Some(0), Some(1)).await;
}
...
async fn dedup_lookup_returns_slot_for_already_applied_seq() {
    ...
    learner.learn(entry, Some(42), Some(3)).await;
    assert_eq!(
...
async fn dedup_tracks_highest_seq_per_client() {
    ...
    learner
        .learn(write_entry(1, &encode_put(b"a", b"1")), Some(7), Some(1))
        .await;
    learner
...
async fn dedup_is_per_client() {
    let learner = PxLearner::new();
    learner
        .learn(write_entry(1, &encode_put(b"k", b"v1")), Some(10), Some(1))
        .await;
    learner
        .learn(write_entry(2, &encode_put(b"k", b"v2")), Some(20), Some(1))
        .await;
...
async fn learn_advances_contiguous_chosen_and_applied_in_order() {
    ...
    for slot in 1..=3u64 {
        learner
            .learn(write_entry(slot, &encode_put(b"k", b"v")), None, None)
            .await;
    }
...
async fn learn_out_of_order_does_not_advance_contiguous_until_gap_filled() {
    ...
    learner
        .learn(write_entry(3, &encode_put(b"k", b"v3")), None, None)
        .await;
...
async fn learn_is_idempotent_for_repeated_slot() {
    let learner = PxLearner::new();
    learner
        .learn(write_entry(1, &encode_put(b"k", b"first")), Some(1), Some(1))
        .await;
    // Re-learn the same slot — watermarks must not advance past 1.
    learner
        .learn(write_entry(1, &encode_put(b"k", b"second")), Some(1), Some(1))
        .await;
    assert_eq!(learner.contiguous_chosen(), 1);
...
async fn noop_entry_advances_watermark_without_mutating_kv() {
    let learner = PxLearner::new();
    learner
        .learn(write_entry(1, &encode_put(b"k", b"v")), None, None)
        .await;
    learner.learn(noop_entry(2), None, None).await;
...
async fn last_chosen_term_tracks_latest_learned_slot() {
    ...
    entry2.term = 9;
    learner.learn(entry2, None, None).await;
