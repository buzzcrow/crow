...
pub fn init_file_logging(
    ...
    let started_at = SystemTime::now()
        ...
        .map_err(|e| {
            format!("system clock is before unix epoch; next step: check host clock: {e}")
        })?
        .as_secs();
    ...
    tracing_subscriber::registry()
        ...
        .map_err(|e| {
            format!(
                "failed to initialize tracing subscriber; next step: initialize logging only once per process: {e}"
            )
        })?;
